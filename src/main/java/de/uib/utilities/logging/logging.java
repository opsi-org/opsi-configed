...

{
	...
	synchronized static final private void init() {
		...
		try {
			...
			// System.getProperty(logging.logging.envVariableForUserDirectory) +
			// File.separator + programSubDir );
			File logDirectory;

			if (System.getenv(logging.windowsEnvVariableAppDataDirectory) != null)
				logDirectory = new File(
						System.getenv(windowsEnvVariableAppDataDirectory) + File.separator + opsiLogdirName);
			else
				logDirectory = new File(
						System.getProperty(envVariableForUserDirectory) + File.separator + programSubDir);
			...
			if (numberOfKeptLogFiles > 0) {
				...
				for (int i = 0; i < numberOfKeptLogFiles; i++) {
					filenames[i] = completeDirectoryString + File.separator + logfileDelimiter + logfileMarker + "___"
							+ i + extension;
	...
	static private boolean checkInit(String mesg) {
		if (!initialized && !noFileAccess) {
			...
			if (wantedDirectory == null
					&& logging_directory_not_yet_set__counter < logging_directory_not_yet_set__counter__maxDisplay) {
	...
	synchronized static public void debugOut(int level, String mesg, boolean modifyingAllowed) {
		// System.out.println("debugOut " + level + ", " + mesg + " allow modifying " +
		// modifyingAllowed);
		if (!checkInit(mesg))
			return;
	...
	synchronized static public void debugOut(Object caller, int level, String mesg) {
		...
		// System.out.println(" " + caller.getClass().getName() + " " + level + " " +
		// mesg);
		...
		// System.out.println("AKT_DEBUG_LEVEL >= LEVEL_INFO || ( debug( modLevel ) &&
		// noFileAccess ) "
		// + AKT_DEBUG_LEVEL + " " + LEVEL_INFO + " " + debug( modLevel ) + " " +
		// noFileAccess);
		...
		if (debug(modLevel) && modLevel < LEVEL_DONT_SHOW_IT) {
			String myText = supplyXInfo(
					makeDebugTextForTime(modLevel, caller.getClass().getName(), shorten(mesg), nowTime));
