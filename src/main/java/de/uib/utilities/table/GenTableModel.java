			...
			implements TableModelFunctions
{
	...
	)
	{
		...
		//filterConditionShowOnlySelected = new DefaultTableModelFilterCondition( keyCol );
		//chainFilter( labelFilterConditionShowOnlySelected, new TableModelFilter( filterConditionShowOnlySelected ) );
	...
	*/
	public void setFilterCondition(TableModelFilterCondition cond)
	{
		...
		//chainedFilter.setInUse( true ); implemented in constructor of TableModelFilter
	}
	...
	*/
	...
	public void produceRows()
	{
		...
		logging.info(this, "produceRows --- using workingfilter  " + workingFilter.getClass().getName() + " : " +  workingFilter );
	...
	protected void setRows( Vector<Vector<Object>> givenRows )
	{
		...
		else
		{
			...
			//myCollator.setStrength(java.text.Collator.PRIMARY); //ignores hyphens as well as capitalization
	...
	*/
	...
	public Object getValueAt(int row, int col)
	{
		...
		else
		{
			...
			//logging.info(this, "getValueAt row " + row + " null? " +  (rows.get(row) == null));
			//logging.info(this, "getValueAt row " + row + " size: " +  rows.get(row).size());
			//logging.info(this, "getValueAt row " + row + " size: " +  rows.get(row).size());
		...
		if (col == colMarkCursorRow) 
			logging.info(this, "getValueAt row, col " + row + ", " + col + " --  cursorrow " + cursorrow + " " + result);
		*/
	...
	public void setCursorRow(int modelrow)
	{
		...
		if ( markCursorRow )
		{
			//refresh of data display is very delicate, this seems to be a working combination of methods
