			...
			implements DocumentListener, KeyListener, ActionListener
{
	...
	public static enum SearchInputType { LINE, PROGRESSIVE };
	 ...
	 * @param int gives the single column ( in natural counting) in the case of single column search
	 * @param String saving of states is activated, the keys are tagged with the parameter
	 */
	public TablesearchPane(SearchTargetModel targetModel, boolean withRegEx, int prefColNo, String savedStatesObjectTag)
	{
	...
	*/
	...
	private void initSavedStates(String savedStatesObject)
	{
		...
		{
			saveSearchpaneProgressiveSearch = new SaveInteger( savedStatesObject + ".progressiveSearch", 0, configed.savedStates);
			saveSearchpaneAllColumnsSearch = new SaveInteger( savedStatesObject + ".allColumnsSearch", 0, configed.savedStates);
			saveSearchpaneFullTextSearch = new SaveInteger( savedStatesObject + ".fullTextSearch", 0, configed.savedStates);
		}
	}
	...
	public void setFiltering( boolean b, boolean withFilterPopup )
	{
		...
		if (withFilterPopup) searchMenuEntries.put( popupMarkAndFilter, true );
	...
	*/
	...
	protected void initComponents()
	{
		...
		*/
		...
		checkmarkSearchProgressive.setToolTipText( configed.getResourceValue("SearchPane.checkmarkSearchProgressive.tooltip") );
		...
		popupNewSearch.addActionListener(new ActionListener()
		                                 {
			                                 public void actionPerformed(ActionEvent e)
			                                 {
				                                 ...
				                                 if (resetFilterModeOnNewSearch) setFilteredMode(false);
		...
		*/
		...
		//unselectedIcon = de.uib.configed.Globals.createImageIcon("images/checked_blue_empty_withoutbox.png", "");
		//selectedIcon = de.uib.configed.Globals.createImageIcon("images/checked_blue_withoutbox.png", "");
		...
		layoutTablesearchPane.setHorizontalGroup(
		    ...
		    .addGroup(layoutTablesearchPane.createSequentialGroup()
		    		...
		    		.addComponent(markReload, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE,GroupLayout.PREFERRED_SIZE)
		              .addGap(Globals.hGapSize/2,Globals.hGapSize/2,Globals.hGapSize/2)
		              .addComponent(navPane, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE,GroupLayout.PREFERRED_SIZE)
		              .addGap(Globals.hGapSize/2,Globals.hGapSize/2,Globals.hGapSize/2)
		              .addComponent(checkmarkSearch, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE,GroupLayout.PREFERRED_SIZE)
		              .addGap(Globals.hGapSize,Globals.hGapSize,Globals.hGapSize)
		              ...
		              .addComponent(labelFilterMarkGap, Globals.hGapSize/2,Globals.hGapSize/2,Globals.hGapSize/2)
		              .addComponent(checkmarkSearchProgressive, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE,GroupLayout.PREFERRED_SIZE)
	...
	public void setSearchFieldsAll()
	{
		...
		if (targetModel != null)
		{
			...
			if ( 
				(saveSearchpaneAllColumnsSearch == null)
				||
	...
	protected int findViewRowFromValue(int startviewrow, Object value, Set colIndices, boolean fulltext)
{
		...
		//	public void run()
			{
				...
				while (foundrow > startFoundrow)
				{
					...
					//logging.info(this, "markAll current selection " + Arrays.toString( targetModel.getSelectedRows() ));
				}
	...
	*/
	...
	{
		logging.info(this, " markAllAndFilter filtering, disabledSinceWeAreInFilteredModel " + filtering + ", " +  disabledSinceWeAreInFilteredMode() );
		if (!filtering)
			return;
	...
	*/
	...
	{
		//logging.info(this, "setRow row, addSelection, select " + row + ", " + addSelection + ", " +select);
		if (select)
		{
			if (addSelection)
				addSelectedRow(row);
	...
	private void searchTheRow(final int startrow, final boolean addSelection, final boolean select)
	{
		...
		//final de.uib.utilities.thread.WaitCursor waitCursor = new de.uib.utilities.thread.WaitCursor(fieldSearch, "TablesearchPane");
	...
	public void keyPressed(KeyEvent e)
	{
		...
		{
			logging.debug(this, "key pressed ENTER on fieldSearch, with content " + fieldSearch.getText()  + " searchInputType " + searchInputType);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getSource() == checkmarkAllColumns)
		{
			...
			if (checkmarkAllColumns.isSelected())
			{
				...
				if (saveSearchpaneAllColumnsSearch != null) saveSearchpaneAllColumnsSearch.serialize( 0 );
			}
			else
			{
				comboSearchFields.setSelectedIndex( preferredColumnIndex );
				if (saveSearchpaneAllColumnsSearch != null) saveSearchpaneAllColumnsSearch.serialize( 1 );
			}
		...
		else if (e.getSource() == checkmarkFullText)
		{
			...
			{
				comboSearchFieldsMode.setSelectedIndex( FULL_TEXT_SEARCH );
				if (saveSearchpaneFullTextSearch != null) saveSearchpaneFullTextSearch.serialize( 0 );
			}
			else
			{
				comboSearchFieldsMode.setSelectedIndex( START_TEXT_SEARCH );
				if (saveSearchpaneFullTextSearch != null) saveSearchpaneFullTextSearch.serialize( 1 );
			}
		...
		else if (e.getSource() == filtermark)
		{
			...
			if (targetModel.isFiltered())
			{
				...
				//logging.info(this, "actionPerformed on filtermark, unfilteredSelection" +  unfilteredSelection);
		...
		{
			logging.debug(this, "actionPerformed on checkmarkSearch, targetModel.isFiltered " + targetModel.isFiltered());
		...
		{
			logging.debug(this,  "actionPerformed on checkmarkSearchProgressiv, set to  " + checkmarkSearchProgressive.isSelected());
			if (checkmarkSearchProgressive.isSelected())
			{
				searchInputType = SearchInputType.PROGRESSIVE;
				if (saveSearchpaneProgressiveSearch != null) saveSearchpaneProgressiveSearch.serialize(0);
			}
			else
			{
				searchInputType = SearchInputType.LINE;
				if (saveSearchpaneProgressiveSearch != null)  saveSearchpaneProgressiveSearch.serialize(1);
			}
