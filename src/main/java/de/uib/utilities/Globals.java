...
{	
	...
	public static final int tableRowHeight = 16; //16 seems to be the default, underscore are not visible; for lyinng inside table cells, 21 is necessary
	...
	public static Integer compareDottedNumberStrings( final String ver1, final String ver2 ) throws NumberFormatException
	{
	...
	public static int compareOpsiVersions(final String number1, final String number2){
		if (number1 == null) throw new IllegalArgumentException("Number1 can not be null");
		if (!number1.matches("[0-9]+(\\.[0-9]+)*")) throw new IllegalArgumentException("Invalid number1 format");

		if (number2 == null) throw new IllegalArgumentException("Number2 can not be null");
		//if (number2 == null) return 1;
		if (!number2.matches("[0-9]+(\\.[0-9]+)*")) throw new IllegalArgumentException("Invalid number2 format");
	...
	public static String makeHTMLlines(String s)
	{
	 ...
	 static private Rectangle getMaxDevice()
 	 ...
 	 static private Rectangle getMinDevice()
 	 {
		...
		for (int j = 0; j < gs.length; j++) {
			...
			for (int i=0; i < gc.length; i++) {
				int w = gc[i].getBounds().width;
				int h = gc[i].getBounds().height;
				//logging.info("minimalBounds: compare " + result + " to " +  gc[i].getBounds());
				if (w < result.width || h < result.height)
					result = new Rectangle(w, h);
			}
		}
 	 ...
 	 static public Rectangle buildLocationOnDefaultDisplay(int intendedWidth, int intendedHeight, int placementX, int placementY)
 	 {
		GraphicsDevice gd = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();
		GraphicsConfiguration gc = gd.getDefaultConfiguration();
		int x =  gc.getBounds().x + placementX;
		int y =  gc.getBounds().y + placementY;
		logging.info("Globals: x " + gc.getBounds().x + " + " + placementX + ", y " + gc.getBounds().y + " + " + placementY);
	...
	static public Rectangle buildLocationOnDefaultDisplay(int intendedWidth, int intendedHeight, int placementX)
 	 {
		...
		logging.info("Globals: x " + gc.getBounds().x + " + " + placementX + ", y " + gc.getBounds().y + " + half of height: " + ((gc.getBounds().height)/2)) ;
		return new Rectangle(  x, y, intendedWidth, intendedHeight);
 	 ...
 	 static public Rectangle buildLocation(javax.swing.JFrame f, int intendedWidth, int intendedHeight, int placementX, int placementY)
 	 {
		...
		logging.info("buildLocation startX, startY, startWidth, startHeight " + startX+ ", " +startY+ ", " +startWidth+ ", " + startHeight);
		...
		else
		{
			 ...
			 if (
				 intendedWidth + 2 * bigFramesDistanceFromLeft > minBounds.width
			 )
			 {
			 ...
			 if (
				 intendedHeight + 2 * bigFramesDistanceFromTop > minBounds.height
			 )
			 {
			 ...
			 {
				 logging.info("buildLocation  width + placementX > minBounds.width " + width + ", " + placementX + ", " + minBounds.width);
			 ...
			 {
				 logging.info("buildLocation  height + placementY > minBounds.height " + height + ", " + placementY + ", " + minBounds.height);
 	 ...
 	 cf. https://stackoverflow.com/questions/2234476/how-to-detect-the-current-display-with-java
 	 
		 GraphicsDevice myDevice = myFrame.getGraphicsConfiguration().getDevice();
		 for(GraphicsDevice gd:GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices()){
			if(
				frame.getLocation().getX() >= gd.getDefaultConfiguration().getBounds().getMinX() &&
	...
	public static void main(String[] args)
	{
		...
		//System.out.println(" result of lastpositionDifferenceForDotSeparatedNumbers " + lastpositionDifferenceForDotSeparatedNumbers(args[0], args[1]));
