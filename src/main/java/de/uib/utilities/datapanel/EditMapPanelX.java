...
// works on a map of pairs of type String - List
{
	...
	protected class SettingDefaultValuesHandler extends PropertyHandler
	{
		...
		{      
			...
			//"set fixed entry"; //                          configed.getResourceValue("EditMapPanelX.PopupMenu.ResetEntryToDefault");
		}
	}
		...
		boolean reloadable)
	{
		...
		logging.debug(this, " created EditMapPanelX instance No " + objectCounter + "::"  +  keylistExtendible + ",  " +  entryRemovable + ",  " + reloadable);
		...
		*/
		...
		if (keylistExtendible || entryRemovable)
		{
			...
			if (entryRemovable) 
			{
				...
				//the crucial point of the different action listeners is that each uses its special property handler
				//which get associated to different menu items (and they are handled each therefore in a specific manner) 
				...
				popupItemDeleteEntry1 = new JMenuItemFormatted(removingSpecificValuesPropertyHandler.getRemovalMenuText(),
				...
				/*
				popupNoEditOptions.addPopupMenuListener( new DatadependentPopupMenuListener(){
						...
						public void popupMenuWillBecomeVisible( PopupMenuEvent e)
						{
							...
							if (rowIndex > -1)
							{
								...
								logging.info(this, "popupMenuWillBecomeVisible compare " + val + " to " + defaultValue);
	...
	*/
	public void removeProperty(String key)
	{
		...
		logging.info(this, " EditMapPanelX instance No " + objectCounter + "::"  + " removeProperty for key " + key + " via  handler " +  propertyHandler);
		
		propertyHandler.removeValue(key);
		
		
		logging.info(this, " EditMapPanelX instance No " + objectCounter + "::"  + " handled removeProperty for key " + key + " options " + optionsMap.get(key)); 
		logging.info(this, "handled removeProperty for key " + key + " default value  " + defaultsMap.get(key) +  " - should be identical with - "   
			+ optionsMap.get(key).getDefaultValues());
