...
public class Messages {
  ...
  private static String findSelectedLocale(String language, String country) {
    ...
    if (existingLocalesNames.indexOf(characteristics) > -1) result =
      characteristics;

    characteristics = language;
    myLocaleCharacteristics.add(characteristics);
    if (
      result == null && (existingLocalesNames.indexOf(characteristics) > -1)
  ...
  public static String getSelectedLocale() {
    if (EXTRA_LOCALE != null) selectedLocaleString =
      EXTRA_LOCALE; else selectedLocaleString =
      findSelectedLocale(myLocale.getLanguage(), myLocale.getCountry());
  ...
  private static int lastIntIndex(String s) {
    ...
    for (int j = 0; j < 10; j++) allowed.append("" + j);
    allowed.append(".");
  ...
  private static void checkUTF8() {
    if (UTF8_HACK == null) {
      ...
      String javaVersionOnlyNumbers = javaVersionOnlyNumbers0.substring(
        0,
        lastIntIndex(javaVersionOnlyNumbers0)
      );

      if (
        javaVersionOnlyNumbers.length() < javaVersionOnlyNumbers0.length()
      ) logging.writeToConsole("shortened to " + javaVersionOnlyNumbers);

      Integer differenceToJava9 = de.uib.utilities.Globals.compareDottedNumberStrings(
        "9",
        javaVersionOnlyNumbers
      );
      logging.writeToConsole(
        " version difference to java 9 is: " + differenceToJava9
      );
  ...
  public static ResourceBundle getResource() throws MissingResourceException {
    ...
    try {
      ...
      if (UTF8_HACK) messages =
        ResourceBundleUtf8.getBundle(BUNDLE_NAME, myLocale); else messages =
  ...
  public static ResourceBundle getResourceEN() throws MissingResourceException {
    ...
    if (UTF8_HACK) messagesEN =
      ResourceBundleUtf8.getBundle(
        BUNDLE_NAME,
        new Locale("en", "US")
      ); else messagesEN =
      ResourceBundle.getBundle(BUNDLE_NAME, new Locale("en", "US"));
    ...
    if (characteristics != null && !characteristics.equals("")) {
      ...
      if (characteristics.length() == 5 && characteristics.indexOf('_') == 2) {
        try {
          loc =
            produceLocale(
              characteristics.substring(0, 2),
              characteristics.substring(3, 5)
            );
          logging.info(
            "Locale " +
            loc.getLanguage() +
  ...
  public static java.util.List<String> getLocaleNames() {
    if (existingLocalesNames == null) getLocaleRepresentations();

    return existingLocalesNames;
  }

  public static Map<String, String> getLocaleInfo() {
    if (localeInfo == null) getLocaleRepresentations();
  ...
  private static java.util.List<LocaleRepresentation> getLocaleRepresentations() {
    if (existingLocales != null) return existingLocales;

    ArrayList<LocaleRepresentation> existingLocales = new ArrayList();
    localeInfo = new TreeMap<String, String>();

    InputStream stream =
      de.uib.messages.Messages.class.getResourceAsStream(LOCALISATIONS_CONF);
    BufferedReader reader = new BufferedReader(new InputStreamReader(stream));
    try {
      String line = reader.readLine();
      while (line != null) {
        line = line.trim();
        if (line.length() > 0 && line.charAt(0) != '#') existingLocales.add(
          new LocaleRepresentation(line)
        );
