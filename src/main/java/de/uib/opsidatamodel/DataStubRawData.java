...
public class DataStubRawData extends DataStubNOM
{
	...
	final static String CELL_SEPARATOR = "|";
		...
		)
	{
		...
		java.util.List<java.util.List<java.lang.String>> 
			rows
				= persist.exec.getListOfStringLists(
					new OpsiMethodCall(
						"getRawData",
						new Object[]{query}
						)
				...
				*/
				...
				else
				{
					...
					*/
					...
					logging.info(this, "retrieveSoftwareAuditOnClients needed for 1st part of constructing entries " + SWAuditClientEntry.summillis1stPartOfConstructor);
					logging.info(this, "retrieveSoftwareAuditOnClients needed for 2nd part of constructing entries " + SWAuditClientEntry.summillis2ndPartOfConstructor);
	...
	*/
	...
	protected void retrieveHostConfigs()
	{
		...
		else
		{
			...
			for (java.util.List<String> row : rows)
			{
				...
				/*Map<String, java.util.List<Object>> configValues1Host = hostConfigValues.get(hostId)	;	
				if (configValues1Host  == null)
				{
					configValues1Host = new HashMap<String, java.util.List<Object>>();
				...
				*/
				...
				//configValues1Host.put( configId,  (java.util.List) configs1Host.get( configId ) );
	...
	private Map<String, Map<String, Object>> client2HwRowsForHwClass(String hwClass)
	{
		...
		{
			if ( 
				hwInfoCol.startsWith( "HOST." )
				||
				hwInfoCol.equals(   persist.lastseenVisibleColName )
				)
				continue; //these already are in the collection
			
			logging.info(this, "hwInfoCol " + hwInfoCol + " look for " + persist.hwInfo_DEVICE + " as well as " + persist.hwInfo_CONFIG);
			String part0 = hwInfoCol.substring(0, persist.hwInfo_DEVICE.length() );
			//logging.info(this, "hwInfoCol part0 " + part0 
			//	+ ", hwInfoCol.substring( part0.length()) " + hwInfoCol.substring( part0.length()));
			...
			else
			{
				...
				if ( !hwInfoCol.substring( part0.length()).startsWith( hwClass ) )
				{
					//logging.info(this, "client2HwRowsForHwClass: not an expected column format: " + hwInfoCol);
		...
		buf.append(", "); buf.append(configTable);
		...
		buf.append(" = ");
		buf.append(configTable); buf.append(persist.hostIdField);
		
		buf.append("\nAND ");
		buf.append(configTable); buf.append(persist.hardwareIdField);
		buf.append(" = ");
		buf.append(deviceTable); buf.append(persist.hardwareIdField);
		
		buf.append("\nAND ");
		buf.append(configTable); buf.append( stateIs1 );
		...
		for (java.util.List<String> row : rows)
		{
			...
			for (int i = 1; i< specificColumns.size(); i++)
			{
				...
				if (specificColumns.get(i).equals(
					lastseenCol) )
				{
		...
		HARDWARE_DEVICE_DISK_PARTITION.hardware_id = HARDWARE_CONFIG_DISK_PARTITION.hardware_id 
	...
	protected void retrieveClient2HwRows(String[] hosts)
	{
		...
		//Map<String, Map<String, Object>> client2AllInfos = new HashMap<String, Map<String, Object>>();
		...
		for ( String hwClass : persist.getHwInfoClassNames() )
		{
			...
			//logging.info(this, "retrieveClient2HwRows client2ClassInfos " + client2ClassInfos);
