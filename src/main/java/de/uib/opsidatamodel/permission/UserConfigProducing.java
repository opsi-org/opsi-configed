	...
	*/
	...
	private void produceRoleAndUserParts(java.util.Set<String> userNames, java.util.Set<String> roleNames)
	{
		...
		for (String key : serverconfigValuesMap.keySet() )
		{
			...
			//logging.info(this, "produceRoleAndUserParts. starting with UserConfig.KEY_USER_ROOT " + key);
	...
	//produces readyObjects for this configKey(List)
	{
		...
		if (serverconfigValuesMap.get( configKeyUseList ) == null)
		{
			...
			item  = PersistenceController.createJSONBoolConfig( configKeyUseList, initialValue, "the primary value setting is " + initialValue );
		...
		if ( (serverconfigValuesMap.get( configKeyList ) == null) 
			...
			|| !currentPossibleValuesListed.equals( oldPossibleValues ))  //includes the case that there was no value for the configKey at all
		{
			logging.info(this, "supplyPermissionList initialization or change");
			
			logging.info(this, "supplyPermissionList add to currentPossibleValuesListed " + currentPossibleValuesListed);
			...
			item.put("description", "the primary value setting is an empty selection list, but all existing items as option");
	...
	protected void supplyAllPermissionEntries(java.util.Set<String> userParts, java.util.Set<String> roleParts)
	{
		...
		//separateServerConfigsTreeSection( UserConfig.KEY_USER_ROOT + "." + UserConfig.ROLE );
		//separateServerConfigsTreeSection( userPart().substring( 0, userPart().length()-1 ) );
		...
		//String username = "admindepot1"; //testing
		{
			...
			logging.info(this, "supplyAllPermissionEntries got values " + values + " for role from serverconfigValuesMap ");

			String configuredRole = null;
			boolean followConfiguredRole = false;
			logging.info(this, "supplyAllPermissionEntries has role " + values);
			//logging.info(this, "supplyAllPermissionEntries is string " + (values.get(0) instanceof String) );
			//logging.info(this, "supplyAllPermissionEntries is NONE " + ( ((String) values.get(0) ).equals( UserConfig.NONE_PROTOTYPE) ));
			...
			logging.info(this, " for user " + username + " followConfiguredRole " + followConfiguredRole + ": " + roleToPlay); 
		...
		)
	{
		...
		logging.info(this, "supplyPermissionEntriesForAUser UserConfig.getUserBoolKeys( "  + UserConfig.getUserBoolKeys());
			...
			//includes at the moment ssh permission keys and opsi permission keys 
		{
			...
			//			String configKey = startkey + UserOpsipermission.PARTKEY_USER_PRIVILEGE_GLOBAL_READONLY ;
			logging.info(this, "supplyPermissionEntriesForAUser boolean configKey " + configKey + " -- partkey " + partkey);
			
			java.util.List<Object> values = serverconfigValuesMap.get( configKey );
			Boolean value = null; 
			
			logging.info(this, "supplyPermissionEntriesForAUser bool configKey " + configKey + " -- partkey " + partkey);
			logging.info(this, "supplyPermissionEntriesForAUser bool configKey has values " + values);

			if (
				values == null || values.size() == 0 || !(values.get(0) instanceof Boolean) 
				//there is no formally correct value)
				|| ( 
					prototypeObligatory && !( (Boolean) values.get(0).equals( prototypeConfig.getBooleanValue( partkey ) ) )
					//the specific values differs from prototype values and must be corrected
					)
				)
				
			{
				logging.info(this, "supplyPermissionEntriesForAUser. serverconfigValuesMap has no value for key " + configKey);
				value = prototypeConfig.getBooleanValue( partkey );
				
				userConfig.setBooleanValue ( partkey, value ); 
				item  = PersistenceController.createJSONBoolConfig( configKey, value, "the primary value setting is based on the user group");
		...
		logging.info(this, "supplyPermissionEntriesForAUser, readyObjects bool keys for user named " + username + " " + readyObjects);
		
		//logging.info(this, "supplyPermissionEntriesForAUser list keys " +  UserConfig.getUserListKeys());
		
		
		//single String valued
		logging.info(this, "supplyPermissionEntriesForAUser UserConfig.getUserStringValueKeys "  + UserConfig.getUserStringValueKeys());
		logging.info(this, "supplyPermissionEntriesForAUser UserConfig.getUserStringValueKeys_withoutRole "  + UserConfig.getUserStringValueKeys_withoutRole());
		...
		for( String partkey : UserConfig.getUserStringValueKeys_withoutRole() )
		{
			...
			{
				logging.info(this, "supplyPermissionEntriesForAUser. serverconfigValuesMap gives not valid value for key " + configKey);
			...
			//logging.info(this, "supplyPermissionEntriesForAUser list configKey " + configKey + " -- partkey " + partkey);
			//logging.info(this, "supplyPermissionEntriesForAUser list " + values);
		}
		
		//Stringlist valued
		
		logging.info(this, "supplyPermissionEntriesForAUser UserConfig.getUserListKeys( "  + UserConfig.getUserListKeys());
		
		/*
		for( String partkey : UserConfig.getUserListKeys() ) 
			//handles for opsi permission keys the associated lists, i.e. the selectedValues  
			//mulitvalue here true!
		{
			...
			logging.info(this, "supplyPermissionEntriesForAUser list configKey " + configKey + " -- partkey " + partkey);
			logging.info(this, "supplyPermissionEntriesForAUser list configKey has values " + values);
			logging.info(this, "supplyPermissionEntriesForAUser list prototype values " + prototypeConfig.getValues( partkey ) );
		...
		*/
		...
		logging.info(this, "supplyPermissionEntriesForAUser, readyObjects list keys for " + username + " " + readyObjects.size());
		...
		String configKeyUseList
		= startkey +  UserOpsipermission.PARTKEY_USER_PRIVILEGE_DEPOTACCESS_ONLY_AS_SPECIFIED;
		
		String partkey =  UserOpsipermission.PARTKEY_USER_PRIVILEGE_DEPOTS_ACCESSIBLE;
		String configKeyList = startkey +  partkey;
		
		boolean defaultvalueForRestrictionUsage 
		= prototypeConfig.getBooleanValue( UserOpsipermission.PARTKEY_USER_PRIVILEGE_DEPOTACCESS_ONLY_AS_SPECIFIED );
		...
		if ( 
			configOptionsMap.get( configKeyList ) == null
			||
		...
		logging.info(this, "depots currentPossibleValuesDepotListed before supplying " + currentPossibleValuesDepotListed);
		...
		configKeyUseList
		= startkey +  UserOpsipermission.PARTKEY_USER_PRIVILEGE_HOSTGROUPACCESS_ONLY_AS_SPECIFIED;
		...
		defaultvalueForRestrictionUsage 
		= prototypeConfig.getBooleanValue( UserOpsipermission.PARTKEY_USER_PRIVILEGE_HOSTGROUPACCESS_ONLY_AS_SPECIFIED );
		...
		logging.info(this, "hostGroups selectedValuesHostgroup before supplying for " + username + ": " + selectedValuesHostgroup);
		logging.info(this, "hostGroups oldPossibleValuesHostgroupListed before supplying for " + username + ": " + oldPossibleValuesHostgroup);
		logging.info(this, "hostgroups possibleValuesHostgroup before supplying for " + username + ": "  + possibleValuesHostgroup);
		logging.info(this, "hostgroups currentPossibleValuesHostgroupListed before supplying for " + username + ": "  + currentPossibleValuesHostgroupListed);
		...
		configKeyUseList
		= startkey +  UserOpsipermission.PARTKEY_USER_PRIVILEGE_PRODUCTGROUPACCESS_ONLY_AS_SPECIFIED;
		...
		defaultvalueForRestrictionUsage 
		= prototypeConfig.getBooleanValue( UserOpsipermission.PARTKEY_USER_PRIVILEGE_PRODUCTGROUPACCESS_ONLY_AS_SPECIFIED );
		...
		{
			oldPossibleValuesProductgroups = new HashSet<Object> (configOptionsMap.get( configKeyList ).getPossibleValues() ) ;
		}
		...
		logging.info(this, "productGroups oldPossibleValuesProductgroupsListed before supplying " + oldPossibleValuesProductgroups);
		logging.info(this, "productGroups currentPossibleValuesProductgroupsListed before supplying " + currentPossibleValuesProductgroupsListed);
		...
		//logging.info(this, "supplyPermissionEntriesForAUser readyObjects " + readyObjects);
		
		if ( countReadyObjectsOnStart == readyObjects.size() )
		{
			logging.info(this, "supplyPermissionEntriesForAUser added no object(s) for saving, for username "  + username);
		}
		else 
		{
			logging.info(this, "supplyPermissionEntriesForAUser added object(s) for saving, for username "  + username + ": " + (readyObjects.size() - 1)  );
			ArrayList< Object> timeVal = de.uib.configed.Globals.getNowTimeListValue( "set by role prototype" );
			
			/*
			ConfigOption itemModifyTime =  PersistenceController.createConfig( ConfigOption.TYPE.UnicodeConfig,
	...
	public static void main(String[] args)
	{
		...
		PersistenceController persist = 
			PersistenceControllerFactory.getNewPersistenceController (theServer, "user", "test");
