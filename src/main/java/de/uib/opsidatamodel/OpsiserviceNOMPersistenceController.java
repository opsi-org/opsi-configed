...
public class OpsiserviceNOMPersistenceController extends PersistenceController
{
	...
	protected Map<String, List<String>> fLicencePool2UnknownSoftwareList; //function pool --> list of assigned software which is not in software table
	...
	*/
		...
		protected void retrieveOpsiHosts()
		{
			...
			if (opsiHostNames == null)
			{
				...
				for (Map<String, Object> host : opsiHosts)
				{
					...
					boolean isConfigserver =  host.get(HostInfo.hostTypeKEY ).equals( HostInfo.hostTypeVALUE_OpsiConfigserver );

					//logging.info(this, "retrieveOpsiHosts  is opsiconfigserver host " + isConfigserver + ": " + name);
				...
				{
					//String message = "no valid authentication or wrong configuration or data corrupt, configserver not found";
					...
					FTextArea f= new FTextArea( null, "opsi configed", true, 
						new String[]{configed.getResourceValue( "PersistenceController.endApp" )
						},
						500, 400);
				...
				for (Map<String, Object> host : opsiHosts)
				{
					...
					)
					{
						...
						if (getConfig(name) != null )
						{
							...
							{
								tested = valueFromConfigStateAsExpected( getConfig(name), key, (Boolean) (wanConfiguration.get( key).get(0) ) );
								if (!tested) break;
							}
							...
							else
							{
								...
								{
									tested = valueFromConfigStateAsExpected( getConfig(name), key, (Boolean) (notWanConfiguration.get( key).get(0)) );
									if (!tested) break;
								}
								if (tested != null && tested)
									result = false;
				...
				else
				{
					...
					*/
					...
					for (String clientName : depot2_host2hostInfo.get(depot).keySet())
					{
						...
						java.util.List<Object> uefiVal = getConfigValueOrDefault( clientName, CONFIG_DHCPD_FILENAME );
			...
			else
			{
				if (locallySavedValueUserRegister != null && locallySavedValueUserRegister )
				{
					...
					FTextArea dialog = new FTextArea(
						...
						new Icon[]{
							(Icon) Globals.createImageIcon("images/checked_withoutbox_blue14.png", ""),
					...
					msg.append("\n" + configed.getResourceValue( "RegisterUserWarning.dialog.info2" ) );//At the moment, user control is not more active!");
					msg.append("\n");
					msg.append("\n" + configed.getResourceValue( "RegisterUserWarning.dialog.option1" ) );//Ignore warning and continue?");
					msg.append("\n" + configed.getResourceValue( "RegisterUserWarning.dialog.option2" ) );//No more warning (locally)?");
					msg.append("\n" + configed.getResourceValue( "RegisterUserWarning.dialog.option3" ) );//Re-activate user check (on the opsi server)?");
					...
					{
						case 1:
							logging.info(this, "setAgainUserRegistration ignore "); break;
	...
	final public void checkConfiguration()
	{
		...
		if (serverPropertyMap.get(KEY_USER_REGISTER) == null || setUserRegisterVal )
		{
			...
			Map<String, Object> item  = createJSONBoolConfig( KEY_USER_REGISTER, KEY_USER_REGISTER_VALUE, "without given values the primary value setting is false");
		...
		ArrayList<Object> readyConfigObjects 
		= new UserConfigProducing(
			applyUserSpecializedConfig(),
			getHostInfoCollections().getConfigServer(),
			getHostInfoCollections().getDepotNamesList(),
			getHostGroupIds(),
			getProductGroups().keySet(),
	...
	public void clearErrorList ()
{
	...
	*/
	...
	{buildValueListsgTreeSection check propertyclass for startkey " + startkey );
		...
		{
			logging.info(this, "separateConfigTreeSection create propertyclass for startkey " + startkey );
	...
	*/
	...
	private boolean checkFullPermission(
		Set<String> permittedEntities, //will be produced
		//final boolean defaultResult,
		final String keyUseList,
	...
	public void checkPermissions()
	{
		...
		depotsFullPermission =checkFullPermission(
			depotsPermitted,
			//true,
			configKeyUseList,
			configKeyList,
		...
		logging.info(this, "checkPermissions depotsFullPermission (false means, depots must be specified " + depotsFullPermission);
		...
		hostgroupsOnlyIfExplicitlyStated = checkFullPermission(
			hostgroupsPermitted,
		...
		productgroupsFullPermission = checkFullPermission(
			productgroupsPermitted,
			//false,
			configKeyUseList,
			configKeyList,
			serverPropertyMap
		...
		//logging.info(this, "checkPermissions productgroups " + getFProductGroup2Members().keySet() );
		...
		logging.info(this, "checkPermissions role " + de.uib.configed.type.user.UserRoleDescription.getRole());
	...
	public  String getStringValueFromItem (Object s)
{
	...
	*/
	...
	{
		return getClientsWithOtherProductVersion(productId, productVersion, packageVersion, false);
	}
	*/

	@Override
	public List<String> getClientsWithOtherProductVersion(String productId, String productVersion, String packageVersion, boolean includeFailedInstallations)
	{
		...
		for (Map<String, Object> m : retrievedList)
		{
			...
			if (
			    ...
			    (
			    	includeFailedInstallations  && InstallationStatus.getLabel(InstallationStatus.UNKNOWN).equals( clientProductState )
			    )
			    ||
	...
	public Boolean isInstallByShutdownConfigured( String host )
	{
		...
		//logging.debug(this, "isInstallByShutdownConfigured for host " + host + " is " + result);
		return result;
	}
	...
	public Boolean isUefiConfigured( String hostname )
	{
		...
		if (
			getConfigs().get(hostname) != null
			&&
			getConfigs().get(hostname).get(CONFIG_DHCPD_FILENAME) != null
			&&
			( (java.util.List) (getConfigs().get(hostname).get(CONFIG_DHCPD_FILENAME) )  ).size() > 0
		)
		{
			String configValue =  (String) ( (java.util.List) (getConfigs().get(hostname).get(CONFIG_DHCPD_FILENAME) ) ).get(0) ;
		...
		(
			getConfigDefaultValues().get( CONFIG_DHCPD_FILENAME ) != null
			&&
			( (java.util.List) (getConfigDefaultValues().get( CONFIG_DHCPD_FILENAME )  ) ) .size() > 0
		)
		{
			String configValue =  (String) ( (java.util.List) (getConfigDefaultValues().get(CONFIG_DHCPD_FILENAME) ) ).get(0) ;
	...
	private Boolean valueFromConfigStateAsExpected(Map<String, Object> configs, String configKey,  boolean expectValue)
	{
		...
		)
		{
			...
			logging.debug(this, "valueFromConfigStateAsExpected configKey, values " + configKey + ", valueList " +  ((java.util.List) (configs.get(configKey) ) )
	...
	protected Boolean findBooleanConfigurationComparingToDefaults ( String host,  Map<String, java.util.List<Object>>defaultConfiguration ) 
	//for checking if WAN default configuration is set
	{
		...
		for  (String key : defaultConfiguration.keySet())
		{
			...
			//logging.info(this, "defaultConfiguration.get( key ).get(0) "  + defaultConfiguration.get( key ).get(0));
			
			tested = valueFromConfigStateAsExpected(  (Map<String, Object>) getConfig(host), key, (Boolean) (defaultConfiguration.get( key ).get(0) ) );
			if (!tested) break;
		}
	...
	protected  Map<String, ConfigOption> getWANConfigOptions()
	{
		...
		Map<String, ConfigOption> allWanConfigOptions = extractSubConfigOptionsByInitial(MetaConfig.CONFIG_KEY +"." + WAN_PARTKEY);

		logging.info(this, " getWANConfigOptions   " +allWanConfigOptions );

		Map<String, ConfigOption> notWanConfigOptions = extractSubConfigOptionsByInitial(MetaConfig.CONFIG_KEY +"." + NOT_WAN_CONFIGURED_PARTKEY + ".");
		//Map<String, ConfigOption> wanConfigOptions = extractSubConfigOptionsByInitial(MetaConfig.CONFIG_KEY +"." + WAN_CONFIGURED_PARTKEY + ".");
		...
		{
			if (
				notWanConfigOptions.get(key).getType() != ConfigOption.TYPE.BoolConfig
			)
			{
	...
	private List<Object> addWANConfigStates(String clientId, boolean wan,  List<Object> jsonObjects)
	{
		...
		//logging.info(this, "addWANConfigState  specifiedConfig " + specifiedConfiguration);
		//logging.info(this, "addWANConfigState  specifiedConfig.keySet() " + specifiedConfiguration.keySet());
		...
		for (String configId : specifiedConfiguration.keySet())
		{
			...
			{
				logging.info(this, "addWANConfigState; until now, no config(State) existed for client " + clientId + " no local update");
	...
	public boolean configureUefiBoot(String clientId, boolean uefiBoot)
	{
		...
		if (result)
		{
			...
			logging.info(this, "configureUefiBoot, configs for clientId " + clientId + " " + getConfigs().get(clientId));
	...
	//opsi 3 comenuItemsOpsiclientdEventmpatibility
{
	...
	*/
	...
	public Map<String, Map<String, String>>  getHostGroups()
	{
		...
		hostGroups
		 = new HostGroups(exec.getStringMappedObjectsByKey(
		                                new OpsiMethodCall(
		                                    "group_getObjects",
		                                    new Object[]{
		                                        callAttributes,
	...
	public boolean setProductGroup(String groupId, String description, Set<String> productSet)
	{
		...
		logging.info(this, "setProductGroup: inOriSetnotInNewSet, inNewSetnotInOriSet. "
	...
	// getHostDescription
{
	...
	*/
	...
	//partial version of produceHwAuditDeviceClasses()
	{
		...
		{
			if (hwAuditClass.get( OpsiHwAuditDeviceClass.CLASS_KEY ) == null || !(hwAuditClass.get(OpsiHwAuditDeviceClass.CLASS_KEY ) instanceof Map) )
			{
				logging.warning(this, "getAllHwClassNames illegal hw config item, having hwAuditClass.get Class " + hwAuditClass.get( hwAuditClass.get(OpsiHwAuditDeviceClass.CLASS_KEY ) ) );
				if ( hwAuditClass.get(OpsiHwAuditDeviceClass.CLASS_KEY) != null )
					logging.warning(this, "getAllHwClassNames illegal hw config item,  hwAuditClass.get Class is of class " + hwAuditClass.get(hwAuditClass.get(OpsiHwAuditDeviceClass.CLASS_KEY).getClass() ) );

				continue;
			}
			String hwClass = (String) ( ((Map)  ( hwAuditClass.get(OpsiHwAuditDeviceClass.CLASS_KEY ) ) ).get(  OpsiHwAuditDeviceClass.OPSI_KEY ) ) ;
				...
				{
					{
						...
						returnedOption = javax.swing.JOptionPane.showOptionDialog(de.uib.configed.Globals.mainFrame,
												 ...
												 + " " + entry.getSWident()
												 + configed.getResourceValue("PersistenceController.reloadSoftwareInformation.question")
	...
	*/
	...
	protected void produceHwAuditDeviceClasses()
	{
		...
		for (Map<String, Object> hwAuditClass : getOpsiHWAuditConf( ) )
		{
			...
			{
				logging.warning(this, "getAllHwClassNames illegal hw config item, having hwAuditClass.get Class " + hwAuditClass.get("Class") );
				if ( hwAuditClass.get(  OpsiHwAuditDeviceClass.CLASS_KEY ) != null )
					logging.warning(this, "getAllHwClassNames illegal hw config item,  hwAuditClass.get Class is of class " + hwAuditClass.get("Class").getClass() );
				if ( hwAuditClass.get(OpsiHwAuditDeviceClass.LIST_KEY) != null )
					logging.warning(this, "getAllHwClassNames illegal hw config item,  hwAuditClass.get Values is of class " + hwAuditClass.get(OpsiHwAuditDeviceClass.LIST_KEY).getClass() );
			...
			hwAuditDeviceClass.setLinuxQuery( (String) ( ((Map)  ( hwAuditClass.get( OpsiHwAuditDeviceClass.CLASS_KEY  ) ) ).get( OpsiHwAuditDeviceClass.LINUX_KEY ) ) );
			hwAuditDeviceClass.setWmiQuery  ( (String) ( ((Map)  ( hwAuditClass.get( OpsiHwAuditDeviceClass.CLASS_KEY  ) ) ).get( OpsiHwAuditDeviceClass.WMI_KEY ) ) );
			...
			hwAuditDeviceClass.setHostConfig(
				configOptions.get("configed.meta_config.wan_mode_off.opsiclientd.event_gui_startup.active")
				);
			...
			*/
			...
			for ( Object m :  (java.util.List) (hwAuditClass.get(OpsiHwAuditDeviceClass.LIST_KEY)) )
			{
				...
				if ( ma.get( OpsiHwAuditDeviceClass.SCOPE_KEY ).equals("i"))
				{
					...
					hwAuditDeviceClass.setHostConfigKey(
						(OpsiHwAuditDeviceClass.CONFIG_KEY + "." +  hwClass + "_" +  OpsiHwAuditDeviceClass.hostAssignedTableType).toLowerCase()
						);
				...
				else if (ma.get( OpsiHwAuditDeviceClass.SCOPE_KEY ).equals("g"))
				{
					...
					hwAuditDeviceClass.setHwItemConfigKey(
						(OpsiHwAuditDeviceClass.CONFIG_KEY + "." +  hwClass + "_" +  OpsiHwAuditDeviceClass.hwItemAssignedTableType).toLowerCase()
						);
				}

				else
				{
					logging.warning(this, "getAllHwClassNames illegal value for key " +  OpsiHwAuditDeviceClass.SCOPE_KEY + " " + ma.get( OpsiHwAuditDeviceClass.SCOPE_KEY ) );
				}
	...
	protected void retrieveClient2HwRowsColumnNames()
	{
		...
		logging.info(this, "retrieveClient2HwRowsColumnNames "
			+ "client2HwRowsColumnNames == null "
			+ (client2HwRowsColumnNames == null)
			);
		if (
			client2HwRowsColumnNames == null
			||client2HwRowsJavaclassNames == null
	...
	private Map<String, Object> produceHwAuditColumnConfig (
		String configKey,
		OpsiHwAuditDeviceClass hwAuditDeviceClass,
		java.util.List<OpsiHwAuditDevicePropertyType> deviceProperties,
		Map<String, Boolean> tableConfigUpdates )
	{
		...
		for (Object value : possibleValues)
		{
			...
			{
				if (
					(tableConfigUpdates.get(value) == null)
					||
					(tableConfigUpdates.get(value) )
					)
				// was in default values and no change, or value is in (old) default values and set again
				{
		...
		Map<String, Object> configItem
		= createJSONConfig( ConfigOption.TYPE.UnicodeConfig,
			configKey, //key
	...
	public boolean saveHwColumnConfig( Map<String, Map<String, Boolean>> updateItems )
	{
		...
		for (String hwClass : hwAuditDeviceClasses.keySet() )
		{
			...
			if (tableConfigUpdates != null)
				logging.info(this, " saveHwColumnConfig tableConfigUpdates  for the host configIdent,  " + tableConfigUpdates);
			
			if (tableConfigUpdates != null)
				//we have got updates for this table configuration
			{

				Map<String, Object> configItem = produceHwAuditColumnConfig(
						configKey,
						hwAuditDeviceClass,
						hwAuditDeviceClass.getDeviceHostProperties(),
						tableConfigUpdates);
				...
				logging.info(this, "saveHwColumnConfig, old configOption for key"
					+ " " + hwAuditDeviceClass.getHostConfigKey()
			...
			if (tableConfigUpdates != null)
				logging.info(this, " saveHwColumnConfig tableConfigUpdates  for the hw configIdent,  " + tableConfigUpdates);
				...
				//we have got updates for this table configuration
			{
				...
				Map<String, Object> configItem = produceHwAuditColumnConfig(
						configKey,
						hwAuditDeviceClass,
						hwAuditDeviceClass.getDeviceHwItemProperties(),
						tableConfigUpdates);
				...
				logging.info(this, "saveHwColumnConfig, we had configOption for key"
		...
		for (String productId : dataStub.getProduct2versionInfo2infos().keySet())
		{
			...
			if (dataStub.getProduct2versionInfo2infos().get(productId) != null)
			{
				...
				if (versionInfo != null && productAllInfos.get(versionInfo) != null)
				{
					...
					aProductInfo.put(
					    OpsiProductInfo.SERVICEkeyLOCKED,
					    productInfo.getLockedInfo()
					);
	...
	public Map getMapOfProductActions (String clientId)
{
	...
	*/
	...
	public Map<String, java.util.List<Map<String, String>>> getMapOfProductStatesAndActions (String[] clientIds)
	{
		...
		if  (states != null)
		{
			//logging.info(this, "getMapOProductStatesAndActions for vbrupertwin7-64 " + states.get("vbrupertwin7-64.uib.local"));
			return states;
		}
	...
	public Map<String, java.util.List<Map<String, String>>>  getMapOfLocalbootProductStatesAndActions (String[] clientIds)
	{
		/*
			...
			*/
			...
			for (String product: localbootProductNames)
			{
				...
				if (withDependencies)
				{
					...
					// logging.debug(this, "resetLocalbootProducts delete propertyStateItem " +  propertyStateItem);
	...
	//not used in opsi 4
{
	...
	*/
	...
	public String getProductVersion(String product)
	{
		...
		/*
		if (!result.equals( exec.getStringValueFromItem ( ((Map) getProductInfos(product)).get ( "productVersion") ) ) )
			logging.error( " productversion " + result + " not equals " +exec.getStringValueFromItem ( ((Map) getProductInfos(product)).get ( "productVersion") ) );
		*/
	...
	private  Map<String, String> getProductRequirements ( String depotId, String productname, String requirementType)
	{
		...
		logging.debug(this, "getProductRequirements depot, productname, requirementType  " + depotId + ", " + productname + ", " + requirementType);
		...
		if (mapOfMethodSignatures == null)
		{
			...
			if (methodsList != null)
			{
				...
				while (iter.hasNext())
				{
					...
					List signature1 = exec.getListFromItem(listEntry.get("params").toString()); // should never result to null
		...
		if (configListCellOptions == null || configOptions == null || configDefaultValues == null)
		{
			...
			while ( tryIt )
			{
				...
				for (Map<String, Object> configItem : retrievedList)
				{
					...
					String pseudouserProducedByOldVersion = 
							KEY_USER_ROOT + ".{"
					...
					if (key != null 
						&& key.startsWith( pseudouserProducedByOldVersion ) )
					{
						logging.warning(this, "user entry " +  key  
						+ " produced by a still somewhere running old configed version , please delete user entry " + pseudouserProducedByOldVersion);
					...
					{
						 if (!configListCellOptions.get(x).isNullable()) logging.info(this, " key " + x + " nullable false ");
					}
					*/
	...
	public  Map<String, ListCellOptions> getConfigOptions()
{
	...
	*/
	...
	//@Override
	protected Boolean getHostBooleanConfigValue( String key, String hostName, boolean useGlobalFallback, Boolean defaultVal )
	{
		Boolean result = null;
		...
		if (
			getConfigs().get( hostName ) != null
			&&
			getConfigs().get(hostName).get( key ) != null
			&&
			((java.util.List) (getConfigs().get( hostName ).get( key ))) .size() > 0 
		)
		{
			
			result = interpretAsBoolean( 
					((java.util.List) (getConfigs().get(hostName).get( key))).get(0),
					(Boolean) null) ;
			
			logging.debug(this, "getHostBooleanConfigValue for key, host " + key + ", " + hostName + " giving " + result ); 
		...
		if (result == null)
		{
			logging.info(this,"got no value for key " + key + " and host " + hostName + " setting default " + defaultVal);
			result = defaultVal;
		}
	...
	public void setGlobalBooleanConfigValue( String key, Boolean val, String description )
	{
			...
			if (exec.doCall(omc))
			{
				...
				//configDefaultValues.put(key, ((org.json.JSONArray)configItem.get("defaultValues")).toList());
			}
	...
	public void hostConfigsCheck(String[] clients)
{
	...
	*/
	...
	/*
	public java.util.List<Object> getConfigValueOrDefault(String host, String key)
	{
		//logging.info(this, "host: getConfigs().get( host ) " + host + ": " + getConfigs().get( host ));
		//logging.info(this, "host, key: getConfigs().get( host ).get( key ) " + host + ", " + key + ": " + getConfigs().get( host ).get( key) );
		logging.info(this, "host: getConfigValues().size() " +  dataStub.getConfigValues().size() );
		logging.info(this, "host: getConfigValues().get( host ) " + host+ ": " + dataStub.getConfigValues().get( host ) );
		logging.info(this, "key: getConfigDefaultValues().get( key ) " + key + ": " + getConfigDefaultValues().get( key ) );
		logging.info(this, "host, key: getConfigValues().get( host ).get( key ) " + host + ", " + key + ": ");
		//logging.info(this, "host, key: " + dataStub.getConfigValues().get( host ).get( key) );


		//if ( getConfigs().get( host )  == null || getConfigs().get( host ).get( key ) == null  || dataStub.getConfigValues().get( host ) == null )
	...
	public Map<String, Map<String, Object>> getConfigs(String[] objectIds)
{
	...
	*/
	...
	/**
	* delivers the default domain
	* if it is not existing it retrieves it from servide
	*
	*/
	@Override
	...
	*/
			...
			for ( Object item : configDefaultValues.get( configedGIVENDOMAINS_key  ) )
			{
				...
				else if (p > 0)  //the only regular case
				{
					...
					try
					{
						...
						if (
							numberedValues.get(value) == null
							||
							orderNumber < numberedValues.get(value)
						)
						// we have a new value; or we take the minimum place if the same value occurs twice
						{
	...
	public Map<String, SWAuditEntry> getInstalledSoftwareInformation()
	{
		...
		for (String key :  dataStub.getInstalledSoftwareInformation().keySet())
		{
			...
			{
				foundEntries.add(  " " + key + " ::: "  +  dataStub.getInstalledSoftwareInformation().get( key ) );
			}
		}
	...
	// without internal caching
{
	...
	*/
	...
	public void retrieveRelationsAuditSoftwareToLicencePools()
	{
		...
		fLicencePool2UnknownSoftwareList = new HashMap<String, List<String>>(); //function pool --> list of assigned software
		...
		for (StringValuedRelationElement  retrieved :  relations_auditSoftwareToLicencePools)
		{
			...
			if (softwareIds.indexOf(swKEY) == -1)
			{
				if ( getInstalledSoftwareInformationForLicensing().get( swKEY ) == null )
				{
					...
					// we serve the fLicencePool2UnknownSoftwareList only in case that a key is found
		...
		logging.info(this, "retrieveRelationsAuditSoftwareToLicencePools,  softwareWithoutAssociatedLicencePool " + softwareWithoutAssociatedLicencePool.size());
	...
	public boolean removeAssociations(
		String licencePoolId,
		List<String> softwareIds)
	{
		logging.info(this, "removeAssociations licensePoolId, softwareIds " +
		licencePoolId + ", " + softwareIds);
	...
	{
		return setWindowsSoftwareIds2LPool(
			licensePoolId, softwareToAssign, false);
	}

	public boolean addWindowsSoftwareIds2LPool(
		String licensePoolId,
		List<String> softwareToAssign
		)
	{
		return setWindowsSoftwareIds2LPool(
			licensePoolId, softwareToAssign, true);
	}


	private boolean setWindowsSoftwareIds2LPool(
	    String licensePoolId,
	    final List<String> softwareToAssign
	    ,boolean onlyAdding
	)
	{
			...
			for (Object swIdent : softwareToAssign)
		{
			...
			*/
			...
			OpsiMethodCall omc = new OpsiMethodCall(
			                         ...
			                         //"auditSoftwareToLicensePool_updateObjects", does not werk in the expected way, cf.redmine #3282)
				...
				//we build the correct data locally
			{
				...
				logging.info(this, "setWindowsSoftwareIds2LPool licencePool, fLicencePool2SoftwareList " + licensePoolId + " : " +  fLicencePool2SoftwareList.get(licensePoolId) );
				...
				for (String ident : newList)
				{
					...
					if (getName2SWIdents().get( swName ) == null) getName2SWIdents().put( swName,  new TreeSet<String>());
					getName2SWIdents().get( swName ).add( ident );
					
					logging.info(this, "setWindowsSoftwareIds2LPool, collecting all idents for a name (even if not belonging to the pool), add ident " + ident + " to set for name " + swName); 
				}
	...
	@Override
	public String editPool2AuditSoftware(
	    String softwareID, String licensePoolID_old, String licencePoolID_new 
	)
	//we have got a SW from software table, therefore we do not serve the unknown software list
	{
		if (!serverFullPermission)
			return "";
		...
		if (withLicenceManagement)
		{
			...
			if (licensePoolID_old != null && 
				!licensePoolID_old.equals( FSoftwarename2LicencePool.valNoLicencepool ) 
			)
			{
			...
			if (ok)
			{
				...
				if (fSoftware2LicencePool != null)
				{
					logging.info(this, "fSoftware2LicencePool.get( softwareID ) "+
						fSoftware2LicencePool.get( softwareID )
					);
...
assert  rowmap.get("SWinventory_used")!= null :  "rowsLicenceStatistics.get(licencePoolId).get(SWinventory_used ) is null";
	...
	{
		String configkey = UserConfig.KEY_USER_ROLE_ROOT + ".{"+ name + "}." 
				+ UserConfig.HAS_ROLE_ATTRIBUT;
	...
	{
		String configkey = UserConfig.START_USER_KEY + name + "}." 
				+ UserConfig.HAS_ROLE_ATTRIBUT;
	...
	private void addRoleAndUserConfig(String configkey, String rolename )
	{
		...
		Map<String, Object> itemRole = PersistenceController.createJSONConfig(
					...
					configkey,
					"which role should determine this configuration",
					false, //editable
					false, //multivalue
				...
				public void run()
					{
						...
						javax.swing.JOptionPane.showMessageDialog(	de.uib.configed.ConfigedMain.dpass,
								info.toString(),
	...
	*/
	...
	//we get a user config value; and set it 
	{
		...
		if ( prototypeConfig != null &&  prototypeConfig.hasBooleanConfig( userconfKey  ) )
		{
			...
			{
				logging.warning(this, "userpart <" + userpart + ">  since no values found setting value for  " + key + " from prototype " +
			...
			else
			{
				...
				logging.info(this, "userpart <" + userpart + "> having value for " + key + " "  +  (Boolean) defaultValues.get(0));;
				//UserSshConfig.KEY_SSH_MENU_ACTIVE_defaultvalue = (Boolean) defaultValues.get(0);
		...
		else if ( prototypeConfig.hasListConfig( userconfKey ) )
		{
			...
			//logging.info(this, "checkStandardConfigs set UserSshConfig.KEY_SSH_MENU_ACTIVE_defaultvalue to " +  (Boolean) defaultValues.get(0));
			//UserSshConfig.KEY_SSH_MENU_ACTIVE_defaultvalue = (Boolean) defaultValues.get(0);
			//UserSshConfig.configDefault.put( UserSshConfig.KEY_SSH_MENU_ACTIVE, (Boolean) defaultValues.get(0));
		}
		else
	...
	*/
	...
	private boolean  checkStandardConfigs()
	{
		...
		if (defaultValues == null)
		{
			item = createJSONBoolConfig( key, 
				DEFAULTVALUE_SEARCH_BY_SQL,
		...
		{
			item = createJSONBoolConfig( key,
				DEFAULTVALUE_CLIENTCONFIG_INSTALL_BY_SHUTDOWN,
		...
		readyObjects.add( produceConfigEntry("UnicodeConfig",
		                                     AdditionalQuery.CONFIG_KEY + "." + key,
		                                     query,
		                                     description)
		                );
		...
		readyObjects.add( produceConfigEntry("UnicodeConfig",
		                                     AdditionalQuery.CONFIG_KEY + "." + key
		                                     + "." + AdditionalQuery.DESCRIPTION_KEY,
		                                     description,
		                                     "")
		                );
		...
		*/
		...
		if (defaultValues == null)
		{
			...
			readyObjects.add( produceConfigEntry("UnicodeConfig",
			                                     key,
			                                     configedWORKBENCH_defaultvalue,
		...
		//logging.info(this, "configDefaultValues.keySet " +  configDefaultValues.keySet());
		...
		if (defaultUserConfigsObsolete.size() > 0)
		{
			exec.doCall (
				new OpsiMethodCall(
					"config_deleteObjects",
					new Object[]{
						exec.jsonArray(defaultUserConfigsObsolete)
					}
				)
			);
		}
	...
	protected ExtendedInteger calculateModulePermission(ExtendedInteger globalMaxClients, final Integer specialMaxClientNumber)
	{
		logging.info(this, "calculateModulePermission globalMaxClients " + globalMaxClients + " specialMaxClientNumber " + specialMaxClientNumber);
	...
	protected void retrieveOpsiLicensingInfoVersion()
	{
		if ( opsiLicensingInfoVersion == null )
		{
			...
			if (  getMethodSignature(backendLicensingInfoMethodname) == NONE_LIST )
			{
				...
				//opsiLicensingInfoVersion = LicensingInfoMap.OPSI_LICENSING_INFO_VERSION_OLD; // test!!!
	...
	private JSONObject retrieveJSONLicensingInfoReduced()
	{
		...
		{
			OpsiMethodCall omc = new OpsiMethodCall(
								backendLicensingInfoMethodname, 
	...
	private void produceOpsiModulesInfo()
	{
		...
		//opsiModulesInfo  = new HashMap<String, Object>(); //the part of opsiinformation which delivers the service information on checked modules
		...
		logging.info(this, "getOverLimitModuleList() " + 
			LicensingInfoMap.getInstance( getOpsiLicensingInfo(), getConfigDefaultValues(), true ).getCurrentOverLimitModuleList()
		);
		...
		else
		{
			if ( licInfoMap.warningExists() )   
			{
				...
				{
					licInfoWarnings =
							new FTextArea(de.uib.configed.Globals.mainFrame,
									configed.getResourceValue("Permission.modules.title"),
									false,
									new String[]{
					...
					mess.append( modulesWithWarning( LicensingInfoMap.CURRENT_OVER_LIMIT, configed.getResourceValue("Permission.modules.warning.currentOverLimit") ));
					mess.append( modulesWithWarning( LicensingInfoMap.CURRENT_CLOSE_TO_LIMIT, configed.getResourceValue("Permission.modules.warning.currentCloseToLimit") ));
					mess.append( modulesWithWarning( LicensingInfoMap.CURRENT_TIME_WARNINGS, configed.getResourceValue("Permission.modules.warning.currentTimeWarning") ));
					//mess.append( modulesWithWarning( LicensingInfoMap.FUTURE_OVER_LIMIT, configed.getResourceValue("Permission.modules.warning.futureOverLimit") ));
					//mess.append( modulesWithWarning( LicensingInfoMap.FUTURE_CLOSE_TO_LIMIT, configed.getResourceValue("Permission.modules.warning.futureCloseToLimit") ));
		...
		//sets value to true if we use the mysql backend and informs that we are underlicensed 
	...
	private void produceOpsiModulesInfoClassic()
	{
		...
		opsiModulesInfo = new HashMap<String, Object>(); //the part of it which delivers the service information on checked modules
		opsiModulesDisplayInfo = new HashMap<String, Object>(); //keeps the info for displaying to the user
		opsiVersion = "4";
		...
		try
		{
			...
			//read in opsi count modules ("realmodules") and overwrite the items if existing
			...
			for (String key : ModulePermissionValue.MODULE_CHECKED.keySet())
			{
				...
				else
				{
					...
					else
					{
						...
						logging.info(this, " retrieveOpsiModules " + key + " maxClientsForThisModule.getNumber " + maxClientsForThisModule.getNumber());
			...
			for (String key : ModulePermissionValue.MODULE_CHECKED.keySet())
			{
				...
				logging.info(this, "check  module " + key + " maxAllowedClientsForThisModule " + maxAllowedClientsForThisModule +  " expiresForThisModule " + expiresForThisModule);
				...
				if (problemToIndicate)
				{
					logging.info(this, "retrieveOpsiModules " + key + " , maxClients " + maxAllowedClientsForThisModule + " count " + countClientsInThisBlock);
					...
					if (
						!ExtendedInteger.INFINITE.equals( maxAllowedClientsForThisModule)
					)
					{
						...
						if(
							countClientsInThisBlock > stopCount 
						)
						{
							logging.info(this, "retrieveOpsiModules " + key + " stopCount " + stopCount + " count clients " + countClients);
							...
							countClientsInThisBlock > startWarningCount  
						)
						{
							logging.info(this, "retrieveOpsiModules " + key + " startWarningCount " + startWarningCount + " count clients " + countClients);
	...
	public boolean isWithScalability1()
{
	...
	*/
	...
	/**
	* return true if opsiversion not meets the requirements (minRequiredVersion) otherwise false
	* (if opsiversion meets requiredversion method returns false)
	*/
	@Override
	public boolean handleVersionOlderThan(String minRequiredVersion)
	{
		//Integer compareResult = de.uib.utilities.Globals.lastpositionDifferenceForDotSeparatedNumbers(opsiVersion, minRequiredVersion);
