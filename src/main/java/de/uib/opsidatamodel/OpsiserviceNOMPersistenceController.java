...
public class OpsiserviceNOMPersistenceController extends PersistenceController {
	...
	private boolean checkFullPermission(Set<String> permittedEntities, // will be produced
			...
			final String keyUseList, final String keyList, final Map<String, List<Object>> serverPropertyMap) {
	...
	private List<Object> addWANConfigStates(String clientId, boolean wan, List<Object> jsonObjects) {
		...
		for (Entry<String, List<Object>> config : specifiedConfiguration.entrySet()) {
			...
			item.put(ConfigStateEntry.VALUES, Executioner.jsonArray(config.getValue()));
	...
	protected List<String> wakeOnLan(Map<String, List<String>> hostSeparationByDepot) {
		...
		for (Entry<String, List<String>> hostSeparationEntry : hostSeparationByDepot.entrySet()) {
			...
			Executioner exec1 = executionerForDepots.get(hostSeparationEntry.getKey());
		...
		for (Entry<String, List<String>> hostSeparationEntry : hostSeparationByDepot.entrySet()) {
			if (hostSeparationEntry.getValue() != null && !hostSeparationEntry.getValue().isEmpty()) {
				...
				if (execsByDepot.get(hostSeparationEntry.getKey()) != null
						&& execsByDepot.get(hostSeparationEntry.getKey()) != Executioner.NONE
						&& !hostsToWake.isEmpty()) {
	...
	public String getBackendInfos() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			((List) backends.get(backendName)).add(listEntry);
		}
		...
		while (backendIterator.hasNext()) {
			...
			List backendEntries = (List) backends.get(backendName);
			...
			while (tryIt) {
				...
				for (Map<String, Object> configItem : retrievedList) {
					...
					for (Entry<String, Object> configItemEntry : configItem.entrySet()) {
						if (configItemEntry.getValue() instanceof org.json.JSONArray) {
							configItem.put(configItemEntry.getKey(),
									((org.json.JSONArray) configItemEntry.getValue()).toList());
						}
					}
	...
	public void setHostValues(Map settings) {
		...
		for (Entry setting : (Set<Entry>) settings.entrySet()) {
			if (setting.getValue() instanceof String
					&& ((String) setting.getValue()).trim().equals(de.uib.opsicommand.JSONReMapper.NULL_REPRESENTER))
				corrected.put(setting.getKey(), org.json.JSONObject.NULL);
			else
		...
		if (Globals.checkCollection(this, "configStateCollection", configStateCollection)
			...
			for (Object stateO : configStateCollection) {
				...
				if (determineConfigOptions) {
					...
					configForUpdate.put("defaultValues", Executioner.jsonArray((List) state.get("values")));
					...
					configForUpdate.put("possibleValues", Executioner.jsonArray(possibleValues));

					// mapping to JSON
					Logging.debug(this, "setAdditionalConfiguation " + configForUpdate);
					callsConfigCollection.add(Executioner.jsonMap(configForUpdate));
				}

				state.put("values", Executioner.jsonArray((List) state.get("values")));

				callsConfigName2ConfigValueCollection.add(Executioner.jsonMap(state));
			}
	...
	private void produceOpsiModulesInfoClassic() {
		...
		opsiModulesInfo = new HashMap<>(); // the part of it which delivers the service information on
											// checked modules
		...
		try {
			...
			for (Entry<String, Object> opsiModuleInfo : opsiModulesInfo.entrySet()) {
				...
				opsiModulesPermissions.put(opsiModuleInfo.getKey(), modulePermission); // first try
			...
			for (Entry<String, Object> opsiCountModule : opsiCountModules.entrySet()) {
				...
				opsiModulesPermissions.put(opsiCountModule.getKey(), modulePermission); // replace value got from modulesInfo

				if (opsiCountModule.getValue() != null)
					opsiModulesDisplayInfo.put(opsiCountModule.getKey(), opsiCountModule.getValue()); // overwrite value from modules by the
	 ...
	 */
	public List<Map<java.lang.String, java.lang.Object>> retrieveCommandList() {
		Logging.info(this, "retrieveCommandList ");

		List<Map<java.lang.String, java.lang.Object>> sshCommands = exec.getListOfMaps(
