...
public class SSHConnectExec extends SSHConnect
{
	...
	public void starting(SSHCommand sshcommand)
	{
		...
		try 
		{
			...
			else
			{
				...
				if (outputDialog != null) outputDialog.appendLater("[" + sshcommand.getId() + "] \t" + configed.getResourceValue("SSHConnection.Exec.cannotAsReadonly"));
			}
		}
		catch (Exception e) 
		{
	...
	{
		logging.info(this, "exec_list commands[" + ((SSHCommand)commands).getId() + "] withGui[" + withGui + "] sequential[" + sequential + "] dialog[" + dialog + "]");
		if (!isConnectionAllowed())
		{
			...
			try {
				...
				final_dia.appendLater("\n["+ configed.getResourceValue("SSHConnection.Exec.dialog.commandlist").trim() +"]    " + defaultCommandsString + "\n\n");
							...
							if (! found_error)
							{
								...
								if (!pmethodHandler.canceled)
								{
									if (co instanceof SSHSFTPCommand)
									{
										...
										exec(co, withGui, final_dia, sequential, rememberPw, commands_list.indexOf(co)+1,  commands_list.size());
								}
