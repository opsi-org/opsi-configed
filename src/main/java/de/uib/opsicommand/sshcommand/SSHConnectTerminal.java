	...
	public SSHConnectTerminal(ConfigedMain main, SSHConnectionTerminalDialog dialog)
	{
		...
		if (dialog == null)
			dialog = new SSHConnectionTerminalDialog(
				configed.getResourceValue("MainFrame.jMenuSSHTerminal")+ " " + SSHConnectionInfo.getInstance().getUser() + "@" + SSHConnectionInfo.getInstance().getHost(), 
				this,
				false /* visible = false*/
			);
		connect();
	...
	public void connect()
	{
		...
		else 
		{
			...
			try
			{
				...
				logging.info(this, "connect user@host " + SSHConnectionInfo.getInstance().getUser() + "@" + SSHConnectionInfo.getInstance().getHost());
				if (SSHConnectionInfo.getInstance().usesKeyfile())
				{
					if (SSHConnectionInfo.getInstance().getKeyfilePassphrase() != "")
						jsch.addIdentity(SSHConnectionInfo.getInstance().getKeyfilePath(), SSHConnectionInfo.getInstance().getKeyfilePassphrase());
					...
					session = jsch.getSession(SSHConnectionInfo.getInstance().getUser(),
						SSHConnectionInfo.getInstance().getHost(), Integer.valueOf(SSHConnectionInfo.getInstance().getPort()));
				}
				else
				{
					session = jsch.getSession(SSHConnectionInfo.getInstance().getUser(),
					...
					logging.info(this, "connect useKeyfile " + SSHConnectionInfo.getInstance().usesKeyfile() + " use password …");
				}
				...
				channel.connect();
				logging.info(this, "connect " + SSHConnectionInfo.getInstance().getUser() + "@" + SSHConnectionInfo.getInstance().getHost());
				dialog.setTitle(SSHConnectionInfo.getInstance().getUser() + "@" + SSHConnectionInfo.getInstance().getHost());
