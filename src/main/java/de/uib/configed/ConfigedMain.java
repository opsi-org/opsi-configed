...
public class ConfigedMain implements ListSelectionListener, TabController, LogEventObserver {
	...
	/*
	 * for each product: a collection of all clients that contains name value pairs
	 * with changed data
	 */
	...
	private boolean lnetbootStatesAndActionsUPDATE = false;
	...
	protected void initGui() {
		...
		displayFieldsLocalbootProducts = new LinkedHashMap<String, Boolean>(
				persist.getProductOnClients_displayFieldsLocalbootProducts());
		displayFieldsNetbootProducts = new LinkedHashMap<String, Boolean>(
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (persist.getGlobalBooleanConfigValue(PersistenceController.KEY_SHOW_DASH_ON_PROGRAMSTART,
	...
	private String getSavedStatesDirectoryName(String locationName) {
		String result = locationName + File.separator + HOST.replace(":", "_");
		return result;
	}
	...
	private Vector<String> readLocallySavedServerNames() {
		...
		try {
			subdirs = savedStatesLocation.listFiles(new FileFilter() {
			...
			for (File folder : subdirs) {
				...
				/*
				 * SavedStates testSavedStates = new SavedStates(checkFile); try{
				 * testSavedStates.load(); } catch(IOException iox) { logging.info(this,
				 * "saved states file could not be loades"); } logging.info(this,
				 * "readLocallySavedServerNames savedStates for " + elementname + " : "
				 * +testSavedStates);
				 */
	...
	public void initDashInfo() {
		...
		logging.info(this, "initDashInfo " + controlDash);
	...
	public void loadDataAndGo() {
		...
		if (opsiExpiresDate != null) {
			...
			if (nowCal.after(noticeCal)) {
				 ...
				 * opsiExpiresDate, configed.getResourceValue("Permission.modules.title"),
		...
		new Thread() {
			public void run() {
				...
				/*
				 * //try to ensure that we get the mainframe SwingUtilities.invokeLater(new
				 * Runnable(){ public void run() { logging.info(this,
				 * "ensuring mainFrame is visible"); mainFrame.setVisible(true); if
				 * (configed.isApplet) { try { Thread.sleep(5000); } catch(InterruptedException
				 * ex) { } adjustSize(10, appletHost.getSize().width-1,
				 * appletHost.getSize().height - 20); } } } );
				 */

			}
		}.start();
		/*
		 * if (reachableUpdater != null) { reachableUpdater.setSuspended(false); }
		 */
	...
	protected void preloadData() { // sets dataReady = true when finished
		...
		/*
		 * test java.util.List<String> startProductList = Arrays.asList( new String[]
		 * {"00_testprod_rupert"}); java.util.List<String> result =
		 * persist.extendToDependentProducts( startProductList, "mydepot.uib.local" );
		 * logging.info(this, " extendToDependentProducts " + result); System.exit(0);
		 */
	...
	public void showExternalDocument(String urlS) {
		try {
			...
			if (configed.isApplet) {
				...
				} else { // Linux, we assume that there is a firefox and it will handle the url
	...
	public void toggleColumnSessionInfo() {
		 ...
		 * setRebuiltPclistTableModel(false, false); selectionPanel.initSortKeys(); if
	...
	public boolean treeViewAllowed() {
		 ...
		 * (Boolean) getOpsiModules().get("treeview")) return true; else { FTextArea f =
		 * new FTextArea(mainFrame, Globals.APPNAME + " - Information", true, 1);
		 * f.setMessage(configed.getResourceValue("ConfigedMain.TreeViewNotActive"));
		 * f.setSize(new Dimension(400, 400)); f.setVisible(true); return false; }
		 */
	}
	...
	public void handleProductActionRequest() {
		...
		/*
		 * if ( persist.isWithLocalImaging()) { startProductActionFrame(); } else {
		 * FTextArea f = new FTextArea(mainFrame, Globals.APPNAME + " - Information",
		 * true, 1); f.setMessage( "not activated");//configed.getResourceValue(
		 * "ConfigedMain.LicencemanagementNotActive")); f.setSize(new Dimension(400,
	...
	private void stoppAt(String location) {
		...
		FTextArea fMessage = new FTextArea(mainFrame, "stopp", true, new String[] { "ok" }, 350, 150);
	...
	protected void actOnListSelection() {
		...
		/*
		 * JOptionPane.showMessageDialog( mainFrame, "list selection value changed",
		 * //"not synchronous", "debug", JOptionPane.OK_OPTION);
		 */
		 ...
		 * manager choices are not more valid && !changeListByToggleShowSelection) //
		 * and not by the show selection toggle {
		 * //GroupsManager.getInstance(this).emptySelectvalues(); logging.info(this,
		 * "empty select values"); }
		 */
		...
		if (mainFrame != null) { // when initializing the program the frame may not exist
			...
			/*
			 * for (String selClient : getSelectedClients()) { if (pcinfos.get(selClient) ==
			 * null) { logging.info(this, "addOnListSelection pcinfo map  null for " +
			 * selClient); //should only occur when a nonexistent client is requested } else
			...
			/*
			 * if (getSelectedClients().length == 1) { try { String selClient =
			 * getSelectedClients()[0]; // if (viewIndex == viewClients) // {
			 * logging.debug(this, "selClient " + selClient + " infos from map " + ((Object)
			 * pcinfos)); logging.info(this, "addOnListSelection selClient " + selClient);
			 * logging.info(this, "addOnListSelection pcinfo map  " +
			 * pcinfos.get(selClient)); if (pcinfos.get(selClient) == null) {
			 * logging.info(this, "addOnListSelection pcinfo map  null for " + selClient);
			 * //should only occur when a nonexistent client is requested } else {
			 * hostInfo.produceFrom( pcinfos.get(selClient).getMap() ) ; } // } } catch
			 * (Exception ex) { logging.warning("valueChanged in ConfigedMain " + ex);
			 * logging.logTrace(ex); // "error occurred  "Ljava.lang.string cannot be cast
			 * do java.lang.string" } mainFrame.setClientInfoediting(true); } else {
			...
			/*
			 * TreePath first = adaptTreeSelection(); if (first != null)
	...
	protected void initMainFrame() { // we call this after we have a PersistenceController
		boolean packFrame = false;

		/*
		 * java.util.List<String> serverList = persist.getServers(); if (serverList ==
		 * null || serverList.size() < 1) { logging.error(" server list empty" );
		 * serverList = new ArrayList(); serverList.add(""); }
		 */
		...
		if (oldSelectedDepots == null) {
			...
			} else { // if none of the old selected depots is in the list we select the config server
				depotsList.setSelectedValue(myServer, true);
			}
		}
		...
		selectionPanel = new JTableSelectionPanel(this) {
			...
			protected void keyPressedOnTable(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_SPACE) { // && ((e.getModifiersEx() & KeyEvent.ALT_DOWN_MASK) ==
															// KeyEvent.ALT_DOWN_MASK))
	...
	protected void initLicencesFrame() {
		...
		/*
		 * is set beforehand: else { Globals.container1 = appletHost.getContentPane(); }
		 */
		...
		/*
		 * licencesFrame.setSize(licencesInitDimension); //center the frame: Dimension
		 * screenSize = Toolkit.getDefaultToolkit().getScreenSize(); Dimension frameSize
		 * = licencesFrame.getSize(); if (frameSize.height > screenSize.height) {
	...
	protected void login(Vector<String> savedServers) { // returns true if we have a PersistenceController and are
														// connected
														/*
														 * if (configed.isApplet) // we should be authenticated, try
														 * connection and produce persistence controller { persist =
														 * DPassword.producePersistenceController(host); return true; }
														 */
		...
		/*
		 * int countWait = 0; while (
		 * !(PersistenceControllerFactory.getConnectionState().getState() ==
		 * ConnectionState.CONNECTED) ) { try { Thread.sleep (1000); countWait++;
		 * logging.info(this, "countWait " + countWait + " waited,  we are in state " +
		 * " " + PersistenceControllerFactory.getConnectionState()); } catch
		 * (InterruptedException ex) { } } //dpass sets a new PersistenceController; we
		 * check it: if ( persist != null && persist.getConnectionState().getState() ==
		 * ConnectionState.CONNECTED ) { dpass.setVisible(false); result = true; }
	...
	private String formatDate(String original) {
		if (original != null) {
			...
			if (ls.length >= 15)
				return (ls[1] + ls[2] + ls[3] + ls[4] + '-' + ls[5] + ls[6] + '-' + ls[7] + ls[8] + ' ' + ls[9] + ls[10]
	...
	/*
	 * private TreePath adaptTreeSelection() { //building tree selection paths
	 * ArrayList<TreePath> selpathlist = new ArrayList<TreePath>(); TreePath
	 * firstPath = null; for ( int j = 0; j < getSelectedClients().length; j++) {
	 ...
	 * //logging.debug(this, " ..  " + listforclient); for ( int k = 0; k <
	 * listforclient.size(); k++) { if (firstPath == null); firstPath =
	 * listforclient.get(k); selpathlist.add(listforclient.get(k)); } }
	 * //selectedTreePaths = selpathlist.toArray(new TreePath[selpathlist.size()]);
	 * //resetSelectedTreeSubjects(); //logging.debug(this, " new tree selection ");
	 * return firstPath; }
	 */
	...
	protected TableModel buildPclistTableModel(boolean rebuildTree) {
		...
		if (allowedClients != null) { // changes the produced unfilteredList
		...
		if (pclist != null) {
			...
			/*
			 * int countDisplayFields = 0; for (String field : host_displayFields.keySet())
			 * { if ( host_displayFields.get( field ) ) countDisplayFields++; }
	...
	/*
	 * one model rebuild too much private boolean
	 * activateGroupWithModelRebuild(String groupname, boolean groupMode) {
	 * activateGroupCounter++; logging.info(this, "activateGroup  " + groupname +
	 * " activateGroupCounter " + activateGroupCounter); if (groupname == null)
	 * return false; if (!treeClients.groupNodesExists() ||
	 * treeClients.getGroupNode(groupname) == null) { logging.warning("no group " +
	 * groupname); return false; } GroupNode node =
	 * treeClients.getGroupNode(groupname); TreePath path =
	 * treeClients.getPathToNode(node); //setGroupByTree(node, path);
	 * activateGroupByTreeWithModelRebuild(node, path); //logging.debug(this,
	 * "clientsFilteredByTree " + clientsFilteredByTree); logging.info(this,
	 * "expand activated  path " + path); treeClients.expandPath(path);
	 ...
	 */
	/*
	 * public boolean activateGroup(String groupname) { return
	 * activateGroupWithModelRebuild(groupname, true); }
	 */
	...
	protected void setRebuiltPclistTableModel(boolean restoreSortKeys, boolean rebuildTree, Set<String> selectValues) {
		...
		/*
		 * if (selectValues == null) { if (rebuiltCounter > 0) { System.exit(0); } else
		 * { rebuiltCounter++; } }
		 */
	...
	private void collectTheProductProperties(String productEdited) { // -- the map of the merged product properties from
																		// combining the properties of all selected
																		// clients // -- // we build
			...
			if (productPropertiesFor1Client != null) {
				...
				for (int i = 1; i < getSelectedClients().length; i++) {
					...
					while (iter.hasNext()) {
						...
						if (mergedProductProperties.get(key) == null) { // we need a new property. it is not common
							ListMerger merger = new ListMerger(value);
	...
	public void setProductEdited(String productname) { // called from ProductSettings
		...
		if (clientProductpropertiesUpdateCollections.get(productEdited) == null) { // if not, we produce one // have we
																					// got already a
		...
		/*
		 * transcode product property descriptions Map productpropertyDescriptionsMap =
		 * persist.getProductPropertyDescriptionsMap(productEdited); if (
		 * productpropertyDescriptionsMap != null &&
		 * !configed.get_serverCharset_equals_vm_charset() ) { Iterator properties =
		 * productpropertyDescriptionsMap.keySet().iterator(); while (
		 * properties.hasNext()) { String property = (String) properties.next(); String
		 * val = (String) productpropertyDescriptionsMap.get (property) ; if (val !=
		 * null && !val.equals("")) { productpropertyDescriptionsMap.put ( property,
		...
		mainFrame.panel_LocalbootProductsettings.initEditing(productEdited, persist.getProductTitle(productEdited),
				...
				persist.getProductTimestamp(productEdited), persist.getProductHavingClientSpecificProperties(), // persist.hasClientSpecificProperties(productname),
				...
				persist.getProductPropertyOptionsMap(productEdited),
				/*
				 * //editmappanel persist.getProductPropertyValuesMap(productEdited),
	...
	/*
	 * private void rebuildSelectedTreeSubjects() { selectedTreePaths =
	 * treeClients.getSelectionPaths(); logging.debug(this,
	 * "rebuildSelectedTreeSubjects: " + selectedTreePaths); TreeSet <String>
	 * selClients = new TreeSet<String>(); if (selectedTreePaths != null) { for (int
	 * i = 0; i <selectedTreePaths .length; i ++) { DefaultMutableTreeNode selNode =
	 * (DefaultMutableTreeNode) selectedTreePaths[i].getLastPathComponent(); String
	 * nodeinfo = (String) selNode.getUserObject(); //logging.debug(this,
	 * "nodeinfo " + nodeinfo); selClients.add(nodeinfo); } } logging.debug(this,
	 * "rebuildSelectedTreeSubjects: " + logging.getStrings(selectedTreePaths) );
	 * setSelectedClientsOnPanel(selClients); if (selClients.size()>0) { Iterator
	 * iter = selClients.iterator(); String client = null; while (iter.hasNext())
	 * client = (String) iter.next(); selectionPanel.moveToValue(client, 0, true);
	 * //selectionPanel.fireListSelectionChanged(treeClients); } } private void
	 * resetSelectedTreeSubjects() { logging.debug(this,
	 * "resetSelectedTreeSubjects: " + logging.getStrings(selectedTreePaths) );
	...
	public boolean treeClients_mouseAction(boolean clicked, MouseEvent mouseEvent) {
		...
		if (mouseRow == -1) {
		} else { // clicked on area for expanding and contracting the node
				...
				if (clicked) {
					if ((activePaths.size() == 1)
							&& ((DefaultMutableTreeNode) activePaths.get(0).getLastPathComponent())
									.getAllowsChildren()) { // on client activation a group activation is ended
						clearTree();
					} else {
	 ...
	 * TreePath pathToNode) { logging.info(this,
	 * "activateGroupByTreeWithModelRebuild, node: " + node + ", pathToNode : " +
	 * pathToNode); setGroupByTree(node, pathToNode); logging.info(this,
	 * "activateGroupByTreeWithModelRebuild, we have selected Set : " +
	 * selectionPanel.getSelectedSet()); //setRebuiltPclistTableModel(true, false,
	 * null); setRebuiltPclistTableModel(true, false); //we set again selected
	 * clients activateGroupByTree(node, pathToNode);
	...
	private void depotsList_valueChanged() {
		...
		/*
		 * if (getSelectedDepots().length == 0) // if nothing is selected set config
		 * server as depot { //logging.debug(this, "depotsList selection set to " +
		 * myServer); //depotsList.setSelectedValue (myServer, true); }
		 */
		 ...
		 * saveSelectedDepotsV); boolean changedSelectedDepotsV =
		 * !selectedDepotsV.equals(saveSelectedDepotsV); logging.info(this,
		 * " selectedDepotsV not changed  " + !changedSelectedDepotsV); if
		 * (!changedSelectedDepotsV) return;
		 */

		/*
		 * test of check of synchronicity TreeSet depotSet = new TreeSet(); for (int i =
		 * 0; i < getSelectedDepots().length; i++) { depotSet.add(getSelectedDepots()[i]
	...
	private boolean checkOneClientSelected() { // enables and disables tabs depending if they make sense in other cases
	...
	protected boolean setLocalbootProductsPage() {
		...
		try {
			...
			if (localbootStatesAndActions == null || istmForSelectedClientsLocalboot == null
					|| localbootStatesAndActionsUPDATE) { // we reload since at the moment we do not track changes if
																																			// anyDataChanged
	...
	protected boolean setNetbootProductsPage() {
		...
		try {
			...
			if (netbootStatesAndActions == null) { // we reload since at the moment we do not track changes if
													// anyDataChanged
													// waitCursor = new WaitCursor( mainFrame.retrieveBasePane(),
													// mainFrame.getCursor() );
	...
	private Map<String, Object> mergeMaps(ArrayList<Map<String, Object>> collection) {
		...
		for (int i = 1; i < collection.size(); i++) {
			...
			for (String key : mergeIn.keySet()) {
				...
				if (mergedMap.get(key) == null) { // new property
					ListMerger merger = new ListMerger(value);
					merger.setHavingNoCommonValue();
					mergedMap.put(key, merger);
				} else {
	...
	protected boolean setProductPropertiesPage() {
		...
		// editingTarget == EditingTarget.SERVER
			 ...
			 * 0 && mainFrame.panel_ProductProperties.paneProducts.getSelectedRowCount() ==
			 * 0 ) { mainFrame.panel_ProductProperties.paneProducts.setSelectedRow(0); }
			 */

			/*
			 * missing product if (depotProductpropertiesUpdateCollection != null) { try
			 * {updateCollection.remove (depotProductpropertiesUpdateCollection);} catch
			 * (Exception ex) {} } depotProductpropertiesUpdateCollection = new
	...
	public boolean setNetworkconfigurationPage() {
		...
		try {
			...
			/*
			 * if (getSelectedClients().length == 0) objectIds = new String[] {myServer};
			 * else objectIds = getSelectedClients();
			 */
			...
			if (editingTarget == EditingTarget.SERVER) {
				...
				/*
				 * if (!checkOneClientSelected()) { viewIndex = viewClients; } else { waitCursor
				 * = new WaitCursor( mainFrame.retrieveBasePane(), mainFrame.getCursor() );
				 * ArrayList additionalConfigs = new ArrayList (1); additionalConfigs.add
				 * (persist.getConfig(selectedClients[0]));//persist.getAdditionalConfiguration(
				 * selectedClients[0])); mainFrame.initNetworkconfigEditing (
				 * selectedClients[0], (Map) additionalConfigs.get(0),
				 * persist.getConfigOptions(), additionalConfigs,
				 * additionalconfigurationUpdateCollection); waitCursor.stop(); }
				 */
	...
	protected void checkSwInfo() {
		/*
		 * if (swInfoClientmap == null) swInfoClientmap = new HashMap<String,
	...
	public void clearSwInfo() {
		/*
		 * checkSwInfo(); swInfoClientmap.clear();
		 */
	}
	...
	protected boolean setLogPage() {
		...
		try {
			/*
			 * if (!checkOneClientSelected()) { viewIndex = viewClients; } else
			 */
			{
	...
	protected boolean setView(int viewIndex) { // extra tasks not done by resetView
		switch (viewIndex) {
		case viewClients: {
			checkErrorList();
	...
	/*
	 * public boolean checkedResetView(int viewIndex) { checkSaveAll(true); return
	 * resetView(viewIndex);6 }
	 */
	...
	public void setViewIndex(int visualViewIndex) {
		...
		/*
		 * if (visualViewIndex != viewClients) { if( clientSelectionDialog != null )
		 * clientSelectionDialog.setEnabled(false); } else { if( clientSelectionDialog
		 * != null ) clientSelectionDialog.setEnabled(true); }
		 */
		...
		if (visualViewIndex != viewClients) {
			if (!((visualViewIndex == viewClients)
				...
				if (getSelectedClients() == null) { // should not occur
				...
				/*
				 * else { if (!setDepotRepresentative()) { //handled in
				 * setLocalbootProductsPage/setNetbootProductsPage problem = true; viewIndex =
				 * viewClients; } }
				 */
			}
		}
		...
		if (!problem && dataReady) { // we have loaded the data
			...
			/*
			 * if (viewIndex != 1) { mainFrame.saveConfigurationsSetEnabled(false); }
			 */
	...
	protected void fetchDepots() {
		...
		/*
		 * new LinkedList<String>(); depotNamesLinked.add(myServer); Map<String, Object>
		 * configServerRecord = new HashMap<String, Object>();
		 * configServerRecord.put("id", myServer); configServerRecord.put("description",
		 * "opsi config server"); if (depots == null || depots.size() < 1) {
		 * logging.warning(" depots list empty, selecting main configuration server" );
		 * depots = new HashMap<String, Map<String, Object>>(); depots.put(myServer,
		 * configServerRecord); } else { depots.put(myServer, configServerRecord);
		 * logging.debug(this, "fetchDepots depots.keySet() " +depots.keySet());
		 * TreeSet<String> depotIdsSorted = new TreeSet<String>( depots.keySet() );
		 * depotIdsSorted.remove(myServer); for (String depotId : depotIdsSorted) {
	...
	private void refreshClientListKeepingGroup() {
		if (dataReady) { // dont do anything if we did not finish another thread for this
			// requestRefreshDataForClientSelection();
	...
	private void reloadData() {
		...
		if (dataReady) { // dont do anything if we did not finish another thread for this
			/*
			 * if( clientSelectionDialog != null) { clientSelectionDialog.leave();
			...
			// for licenses
			/*
			 * persist.getRelationsWindowsSoftwareId2LPool(); Iterator iter =
			 * allControlMultiTablePanels.iterator(); while (iter.hasNext()) {
			 * ControlMultiTablePanel cmtp = (ControlMultiTablePanel) iter.next(); if (cmtp
			 * != null) { for (int i = 0; i < cmtp.getTablePanes().size(); i++) {
			 * PanelGenEditTable p = cmtp.getTablePanes().get(i); p.reload(); } } }
			 */
			fetchDepots();
		...
		if (selectionPanel != null) { // no action before gui initialized
			...
			if (clientsLeft.size() == 0)
				selectionPanel.fireListSelectionEmpty(this); // no list select item is provided
	...
	public class GeneralDataChangedKeeper extends DataChangedKeeper {
		...
		public boolean askSave() {
			...
			if (this.dataChanged) {
				...
				/*
				 * try { fAskSaveProductConfiguration.setAlwaysOnTop(true); }
	...
	public class HostConfigsDataChangedKeeper extends GeneralDataChangedKeeper {
		...
		public HostConfigsDataChangedKeeper() {
			...
			super();
			/*
			 * setActingOnSource( source -> { logging.info(this, "changed source " + source
			 * ); logging.info(this, "is Stsring " + (source instanceof String) ); String[]
			 * parts = source.toString().split(","); for (String line: parts) {
	...
	public int checkClose() { // 2 cancel closing // 1 lose changes // 0 save changes
	...
	private class ReachableUpdater extends Thread {
		...
		public void run() {
			while (true) {
				logging.debug(this, " " + " suspended , editingTarget, viewIndex " + suspended + ", " + editingTarget
						+ ", " + viewIndex);
					...
					try {
						...
						if (persist.getHost_displayFields().get("clientConnected")) {
							...
							for (int row = 0; row < model.getRowCount(); row++) {
								...
								/*
								 * could be sufficient: if (!model.getValueAt(row, col).equals(newInfo)) {
								 * //changed = true; model.setValueAt(newInfo, row, col);
						...
						/*
						 * java.util.List selectedClients = getSelectedClientsInTable(); if (changed)
						 * model.fireTableDataChanged(); if (selectedClients != null &&
						 * selectedClients.size() > 0) setSelectedClientsOnPanel(selectedClients);
						 */
					} catch (Exception ex) {
	...
	public void callNewClientDialog() {
		 ...
		 * ((java.util.List)persist.getMethodSignature("createClient"))); if (
		 * ((java.util.List)persist.getMethodSignature("createClient")).contains(
		 * "hardwareAddress") )
		 * NewClientDialog.getInstance(this).setMacAddressFieldVisible(true); else
	...
	/*
	 * transferred to persistencecontroller protected void
	 * setDepotForClients(String[] clients, String depotId) { Map<String, Object>
	 * values = new HashMap<String, Object>(); ArrayList depots = new ArrayList();
	 * values.put("clientconfig.depot.id", depots); ConfigName2ConfigValue config =
	 * new ConfigName2ConfigValue(null); //logging.debug(this,
	 * "setDepotForClients, values " + values); depots.add(depotId);
	 * config.put("clientconfig.depot.id", depots); for (int i = 0; i <
	 * clients.length; i++) { //collect data logging.debug(this,
	 * "setDepotForClients, client " + clients[i] + ", configState " + config);
	 * persist.setAdditionalConfiguration(clients[i], config); } //send data
	 ...
	 * !depotId.equals("")) { Map networkConfig =
	 * persist.getNetworkConfiguration(clientId); networkConfig.put("depotId",
	 * depotId); persist.setNetworkConfiguration(clientId, networkConfig); } }
	 */

	/*
	 * protected void restrictClientAccess() { logging.info(this,
	 * "restrict client access start"); allowedClients = null; if
	 * (TEST_ACCESS_RESTRICTED_HOST_GROUP == null) return; Set<String> acceptedHosts
	 * = new HashSet<String>(); for (String client :
	 * persist.getHostInfoCollections().getFNode2Treeparents().keySet()) {
	 * //logging.info(this, "initGui parents for client " + client + " :: " //+
	 * persist.getHostInfoCollections().getFNode2Treeparents().get( client )); if
	 * (persist.getHostInfoCollections().getFNode2Treeparents().get( client
	 * ).contains ( TEST_ACCESS_RESTRICTED_HOST_GROUP ) ) { acceptedHosts.add(
	 * client ); //logging.info(this, "initGui getSimpleTreePaths for " + client +
	 * " :: " + treeClients.getSimpleTreePaths( client ) ); } } logging.info(this,
	 * "restrict client access " + acceptedHosts); if (acceptedHosts.size() > 0)
	...
	public void createClient(final String hostname, final String domainname, final String depotID,
		...
		/*
		 * new Thread (){ public void run() { if (persist.createClient(hostname,
		 * domainname, description, notes, ipaddress, macaddress)) { final String
		 * newClientID = hostname + "." + domainname; if (depotID != null &&
		 * !depotID.equals("")) setDepotForClients(new String[]{newClientID}, depotID);
		 * //is guarantueed by a config //else // setDepotForClients(new
		 * String[]{newClientID}, myServer); checkErrorList();
		 * persist.addOpsiHostName(newClientID); try { SwingUtilities.invokeLater(new
		 * Runnable() { public void run() { refreshClientList(newClientID); } } ); }
		 * catch(Exception iex) { logging.info(this, "refreshClientList " + iex); } }
		 * waitCursor.stop(); } }.start();
		 */
	...
	/*
	 * new Thread(){ public void run() { FShowList fListFeedback = new
	 * FShowList(mainFrame, "wake on lan", false, 0);
	 * fListFeedback.setFont(Globals.defaultFont); fListFeedback.setMessage("");
	 * fListFeedback.setButtonsEnabled(false); Cursor oldCursor =
	 * fListFeedback.getCursor(); //fListFeedback.setCursor(new
	 * Cursor(Cursor.WAIT_CURSOR)); fListFeedback.setVisible(true);
	 * fListFeedback.glassTransparency(true, 1000, 200, 0.04f); for (int i = 0; i <
	 * getSelectedClients().length; i++) {
	 * fListFeedback.appendLine("trying to start " + getSelectedClients()[i]);
	 * persist.wakeOnLan(new String[] {getSelectedClients()[i]} ); try {
	 * Thread.sleep(1000 * delaySecs); } catch(InterruptedException ies) { } }
	 * //fListFeedback.setVisible(false); //fListFeedback.dispose(); //fListFeedback
	 * = null; } }.start();
	 */
	...
	public void fireOpsiclientdEventOnSelectedClients(final String event) {
		...
		/*
		 * without feed back other than general waiting bar new Thread(){
		 * @Override public void run() { persist.fireOpsiclientdEventOnClients(event,
		 * getSelectedClients()); } }.start();
		 */
	...
	public void showPopupOnSelectedClients(final String message) {
		...
		new ErrorListProducer(configed.getResourceValue("ConfigedMain.infoPopup") + " " + message) {
			@Override
			protected java.util.List getErrors() {
				return persist.showPopupOnClients(message, getSelectedClients());
			}
		}.start();
	}
	...
	public void clientSelectionGetSavedSearch() {
		...
		/*
		 * Object value = savedSearchesDialog.getValue(); logging.debug( this,
		 * "clientSelectionGetSavedSearch value "+value ); if( value == null ) return;
	...
	public void callClientSelectionDialog() {
		...
		/*
		 * if( clientSelectionDialog == null ||
		 * clientSelectionDialog.isReloadRequested()) { if (clientSelectionDialog !=
		 * null) { clientSelectionDialog.leave(); } clientSelectionDialog = new
		 * ClientSelectionDialog(this, selectionPanel, savedSearchesDialog); }
		 */
		...
		clientSelectionDialog.setVisible(true);
		/*
		 * try { clientSelectionDialog.setAlwaysOnTop(true); } catch (SecurityException
		 * ex) { logging.debug(this, "callClientSelectionDialog " + ex); }
		 */
	}
	 ...
	 * packageVersion, Map hwFilter) // retrieves the clients fitting to the
	 * conjunction of criteria // deselects everything if all strings are null or
	 * empty { String actionRequest_querystring = ""; if (!actionRequest.equals(""))
	 * actionRequest_querystring = actionRequest; //ActionRequest.getIDString(
	 * ActionRequest.getTypeFromDisplayString (actionRequest, ardDefault)) ; //Map
	 * pclist =
	 * persist.getHostInfoCollections().getPcListForDepots(getSelectedDepots());
	 * //logging.debug(this, " ---------- pclist before makeClientSelection " +
	 * pclist); persist.makeClientSelection("", depotsList.getSelectedValues(),
	 * groupId, productId, installationStatus, actionRequest_querystring,
	 * productVersion, packageVersion, hwFilter); setClientGroup();
	 * checkErrorList(); }
	 */
	...
	public void logEventOccurred(LogEvent event) {
		 ...
		 * logging.checkErrorList(licencesFrame); } else {
		 * logging.checkErrorList(mainFrame); }
		 */
