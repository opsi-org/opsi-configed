...
public class ConfigedMain
			implements 
			ListSelectionListener, TabController, LogEventObserver
			
{
	...
	final static String TEST_ACCESS_RESTRICTED_HOST_GROUP = null;
	...
	protected Map<String, java.util.List<String>> possibleActions = new HashMap<String, java.util.List<String>>(); // a map of products, product --> list of actions; used as an indicator that a product is in the depot
	...
	final public Dimension licencesInitDimension 
	...
	public ConfigedMain(JApplet appletHost, String host, String user, String password)
	{
		...
		SSHConnectionInfo.getInstance().useKeyfile(
			SSHKEY != null ? true:false,
			SSHKEY != null ? SSHKEY:"",
	...
	protected void initGui()
	{
		...
		SwingUtilities.invokeLater(new Runnable(){
			                           ...
			                           public void run()
			                           {
				                           ...
				                           if (persist.getGlobalBooleanConfigValue( 
	...
	private String getSavedStatesDefaultLocation()
	{
		...
		{
			result = System.getenv(logging.windowsEnvVariableAppDataDirectory) + File.separator + logging.creatorDirName + File.separator +  logging.programSubDir;
		}
		else
		{
	...
	private void initSavedStates()
	{
		...
		if (configed.savedStatesLocationName != null)
		{
			...
			try
			{
				...
				configed.savedStates = new de.uib.utilities.savedstates.SavedStates(new File(savedStatesDir.toString()
	...
	// sets dataReady = true when finished
	{
		...
		/* test 
		java.util.List<String> startProductList = Arrays.asList( new String[] {"00_testprod_rupert"});
		java.util.List<String> result = persist.extendToDependentProducts( startProductList, "mydepot.uib.local" );
	...
	public void toggleColumnInstallByShutdownActive()
	{
		...
		{
			JOptionPane.showMessageDialog(	mainFrame, 
				  "An older configed is running in the network", 
				  "Information",
	...
	public void handleLicencesManagementRequest()
	{
...

		{
			...
			de.uib.opsidatamodel.modulelicense.FOpsiLicenseMissingText.callInstanceWith( configed.getResourceValue("ConfigedMain.LicencemanagementNotActive" ) );
	...
	protected void actOnListSelection()
	{
		...
		*/
		...
		if ( mainFrame != null ) // when initializing the program the frame may not exist
		{
			...
			logging.info(this, "actOnListSelection, produce hostInfo  getSelectedClients().length "+ getSelectedClients().length);
			...
			*/
			...
			mainFrame.getHostsStatusInfo().updateValues(pcCount, getSelectedClients().length, 
				getSelectedClientsStringWithMaxLength( HostsStatusPanel.maxClientnamesInField ),   
				clientInDepot );
	...
	private void locateAndDisplay()
	{
		...
		logging.info(this, "locateAndDisplay, call buildLocation startX, startY, startWidth, startHeight, got " + savedX+ ", " +savedY+ ", " +savedWidth+ ", " + savedHeight);
	...
	protected void initLicencesFrame()
	{
		...
		*/
		...
		licencesFrame.setSize( licencesInitDimension );
		final Rectangle dim = de.uib.utilities.Globals.buildLocation(
			licencesInitDimension.width,
			licencesInitDimension.height,
			0, 0);
	...
	protected Map<String, Boolean> producePcListForDepots(String[] depots,Set<String> allowedClients )
	{
		...
		logging.info(this, " producePcListForDepots " + logging.getStrings(depots) + " running with allowedClients " + allowedClients); 
	...
	private TreePath adaptTreeSelection()
{
	...
	*/
	...
	protected TableModel buildPclistTableModel(boolean rebuildTree)
	{
		...
		if (rebuildTree)
		{
			...
			allowedClients = treeClients.associateClientsToGroups( allPCs, persist.getFObject2Groups(),  permittedHostGroups );
			...
			//changes the produced unfilteredList
		{
			...
			if (rebuildTree)
			{
				...
				treeClients.associateClientsToGroups( allPCs, persist.getFObject2Groups(), 
					null //we got already allowedClients, therefore don't need the parameter hostgroupsPermitted
					);
				//treeClients.associateClientsToGroups( allPCs, persist.getFObject2Groups() );
	...
	*/
	...
	public void requestReloadStatesAndActions(boolean onlyUpdate)
	{
		...
		//currentSortKeysLocalbootProducts = mainFrame.panel_LocalbootProductsettings.getSortKeys();
		//currentSortKeysNetbootProducts = mainFrame.panel_NetbootProductsettings.getSortKeys();
	...
	*/
	...
	private void setSelectionPanelCols()
	{
		...
		if (persist.getHost_displayFields().get(HostInfo.clientConnected_DISPLAY_FIELD_LABEL))
		{
			...
			column.setCellRenderer(new BooleanIconTableCellRenderer(
			                          // Globals.createImageIcon("images/ok22.png", ""),
			                           ...
			                           Globals.createImageIcon("images/new_network-empty22.png", ""),
			                           false
			                       )
			                      );
		...
		if (persist.getHost_displayFields().get( HostInfo.clientInstallByShutdown_DISPLAY_FIELD_LABEL ) )
		{
			...
			logging.info(this, "showAndSave columns are " + columns + ", search for " + HostInfo.clientInstallByShutdown_DISPLAY_FIELD_LABEL);
			
			
			
			int col = selectionPanel.getTableModel().findColumn(
			              configed.getResourceValue("ConfigedMain.pclistTableModel." + HostInfo.clientInstallByShutdown_DISPLAY_FIELD_LABEL)
			          );
	...
	//called from ProductSettings
	{
		...
		*/
		...
		mainFrame.panel_LocalbootProductsettings.initEditing(
		    ...
		    persist.getProductVersion(productEdited) 
		    	+ Globals.ProductPackageVersionSeparator.forDisplay() 
	...
	*/
	...
	public boolean treeClients_mouseAction(boolean clicked, MouseEvent mouseEvent)
	{
				...
				if (clicked)
				{
					...
					{							
						mainFrame.getHostsStatusInfo().setGroupName( 
							 ((DefaultMutableTreeNode) mouseNode).getParent().toString()
							 );
					}
					else 
					{
		...
		if(mouseEvent.getClickCount() == 2) {
			...
			logging.debug(this, "treeClients: mouseNode instanceof GroupNode " + mouseNode + " " + (mouseNode instanceof GroupNode));
	...
	{
		//WaitCursor waitCursor = new WaitCursor( de.uib.configed.Globals.mainContainer, "setClientByTree" );
	...
	*/
	...
	protected boolean setLocalbootProductsPage()
	{
		...
		try
		{
			...
			java.util.List<? extends RowSorter.SortKey>  currentSortKeysLocalbootProducts = mainFrame.panel_LocalbootProductsettings.getSortKeys();
	...
	protected boolean setNetbootProductsPage()
	{
		...
		Set<String> savedFilter
		=  (Set<String>) configed.savedStates.saveNetbootproductFilter.deserialize();
		...
		try
		{
			...
			java.util.List<? extends RowSorter.SortKey> currentSortKeysNetbootProducts = mainFrame.panel_NetbootProductsettings.getSortKeys();
			...
			try
			{
				...
				logging.info(this, "resetFilter " +  configed.savedStates.saveLocalbootproductFilter.deserialize());
				//( (InstallationStateTableModelFiltered) istmForSelectedClientsNetboot ).resetFilter();
	...
	protected boolean setProductPropertiesPage()
	{
		...
		if (
			editingTarget == EditingTarget.DEPOTS
			//||
	...
	{
		logging.info(this, "setHardwareInfoPage for, clients count " +
		...
		try
		{
			...
			if (pcName == null || getSelectedClients().length == 0)
			{
				mainFrame.setHardwareInfoNotPossible(
					configed.getResourceValue("MainFrame.noClientSelected1"),
					null
					);
			}		
			else if (getSelectedClients().length > 1)
			{
				...
				{
					mainFrame.setHardwareInfoNotPossible(
						configed.getResourceValue("MainFrame.backendSQLrequired1"),
	...
	private void reloadData()
	{
			...
			// dont do anything if we did not finish another thread for this
		{
			...
			*/
			...
			if (mainFrame != null 
				&& mainFrame.fDialogOpsiLicensingInfo != null
			...
			while (iter.hasNext())
		{
			...
			*/
			fetchDepots();
			//persist.checkConfiguration();  // we do this again since we reloaded the configuratio
	...
	*/
	public class ClientInfoDataChangedKeeper extends DataChangedKeeper
	{
		...
		public void dataHaveChanged(Object source1)
		{
			...
			{
				logging.debug(this, "source empty, we set datachanged false, source " + source);
			...
			if (this.dataChanged)
			{
				if (fAskSaveChangedText== null)
				{
					...
					fAskSaveChangedText.setMessage(
					    configed.getResourceValue("MainFrame.SaveChangedValue"));
	...
	public void checkSaveAll(boolean ask)
	{
		...
		if (anyDataChanged)
		{
			...
			if
			( !ask
			        || hostConfigsDataChangedKeeper.askSave()
			)
				hostConfigsDataChangedKeeper.save();
			...
			public void run()
			{
				...
				if (persist.getHost_displayFields().get("clientConnected"))
				{
					...
					for (int row = 0; row < model.getRowCount(); row ++)
					{
						...
						//logging.info(this, clientId + " reachable " + reachableInfo.get( clientId ) + " old value was " + model.getValueAt(row, col) );
		...
		try
		{
			...
			new Thread()
			{
				public void run()
				{
					...
					{
						logging.info(this, "invocation target or interrupt ex at  iconButtonSessionInfo.setEnabled(false) "
						+ ex);
					}
	...
	public void callNewClientDialog()
	{
		...
		*/
		...
		NewClientDialog.getInstance().useConfigDefaults(
			persist.isInstallByShutdownConfigured( myServer ),
	...
	protected void setDepotForClient(final String clientId, final String depotId)
{
	...
	*/
	...
	protected void restrictClientAccess()
	{
		...
		for (String client : persist.getHostInfoCollections().getFNode2Treeparents().keySet())
		{
			//logging.info(this,  "initGui parents for client " + client + " :: " 
				//+ persist.getHostInfoCollections().getFNode2Treeparents().get( client ));
			if (persist.getHostInfoCollections().getFNode2Treeparents().get( client ).contains ( TEST_ACCESS_RESTRICTED_HOST_GROUP ) )
			{
				acceptedHosts.add( client );
				//logging.info(this, "initGui getSimpleTreePaths for " + client + " :: " +  treeClients.getSimpleTreePaths( client ) );
			}
	...
	*/
	...
	public void setInstallByShutdownProductPropertyValue(boolean newStatus)
	{
		...
		{
			persist.setCommonProductPropertyValue( new HashSet(Arrays.asList( getSelectedClients() ) ), product, "on_shutdown_install" , Arrays.asList(activate) );
	...
	public void setInstallByShutdownProductPropertyValue(String clientname, boolean status)
	{
		...
		// if ( (shutdown_value.get(0) != null) && !(shutdown_value.get(0).equals(aktivate))  )
		{
			persist.setCommonProductPropertyValue( new HashSet(Arrays.asList(clientname)), product, "on_shutdown_install" , Arrays.asList(activate) );
	                         ...
	                         final String group, final String productNetboot, final String productLocalboot)
	{
		...
		logging.debug(this, "createClient " + hostname + ", " + domainname + ", " + depotID + ", " + 
				description + ", " + inventorynumber + ", " + notes  + 
				shutdownInstall + ", " + uefiBoot +  ", " + wanConfig + ", " +
	...
	*/
	...
	public void selectClientsNotCurrentProductInstalled(java.util.List<String> selectedProducts, boolean includeClientsWithBrokenInstallation)
	{
		...
		java.util.List<String> clientsToSelect = persist.getClientsWithOtherProductVersion(productId,productVersion, packageVersion, includeClientsWithBrokenInstallation);
		...
		logging.info(this, "selectClientsNotCurrentProductInstalled clients found for displayed client list " + clientsToSelect.size());
