	...
	IFInstallationStateTableModel
{
	...
	*/
	...
	protected Map<String, Map<String, Map<String, String>>>allClientsProductStates; // (clientId -> (productId -> (product state key -> product state value)))
	//final protected Map<String, java.util.List<Map<String, String>>> allClientsProductlistsSaved;//clientId -> (productrows)
		...
		for (String clientId : allClientsProductStates.keySet())
		{
			for (String productId : allClientsProductStates.get( clientId).keySet() )
			{
				...
				if (!stateAndAction.get("actionRequest").equalsIgnoreCase("none"))
					logging.info(this, "rebuildVisualStatesFromExistingEntries stateAndAction for client " + clientId + " product "  + productId 
	...
	*/
	...
	private void produceVisualStatesFromExistingEntries()
	{
		...
		for (String clientId : allClientsProductStates.keySet())
		{
			for (String productId : allClientsProductStates.get( clientId ).keySet() )
			{
				...
				{
					logging.warning(this, "produceVisualStatesFromExistingEntries, no row for " + clientId + ", " + productId);
					continue;
				}
				...
				for (String colKey : ProductState.KEYS) 
				{
					...
					//logging.debug(this, "produceVisualStates, clientId " + clientId + ", combinedVisualValues.get(colKey)  " + combinedVisualValues.get(colKey));
						
					//if (stateAndAction.get(ProductState.KEY_actionRequest).equals("setup"))
					//	logging.info(this, "produceVisualStatesFromExistingEntries clientId " + clientId + ", stateAndAction " +  stateAndAction);
	...
	//protected String mixToVisualState (final String columnName, Map<String, String> visualStates, final String productId, final String mixinValue, boolean initial)
	
	protected String mixToVisualState (final String columnName, Map<String, String> visualStates, final String productId, final String mixinValue)
	{
		String oldValue = (String) visualStates.get(productId);
		//logging.info(this, " -------------------  mixtovisualstates, columnName " + columnName + "  productId " + productId + " oldValue " + oldValue + "  mixinValue " + mixinValue);
		...
		//! states.containsKey(productId) 
		{
			...
			//	logging.info(this, " -------------------  mixtovisualstates, columnName " + columnName + "  productId " + productId + " oldValue null " + " mixinValue " + mixinValue);
	...
	private void checkForContradictingAssignments( String clientId, String product,  String stateType, String state )
	{
		...
		if (existingRequest == null || existingRequest.equals("") )
		{
			...
			logging.debug(this, "checkForContradictingAssignments client " + clientId + ", actualproduct " + actualProduct + ", product " + product + ", stateType " + stateType + ", state " + state);
		...
		else
		{
			...
			info = info + " for onGoingCollectiveChangeEventCount " + onGoingCollectiveChangeEventCount + " contradicting " + contradicting;
			if (contradicting) 
			{
					...
					new Thread(){
						public void run()
						{
							javax.swing.JOptionPane.showMessageDialog(	de.uib.configed.Globals.mainFrame,
								infoOfChange,
								configed.getResourceValue("InstallationStateTableModel.contradictingProductRequirements.title"),
				...
				else
				{
					...
					logging.warning(this, "checkForContradictingAssignments " + info + " client " + clientId + ", actualproduct " + actualProduct + ", product " + product + ", stateType " + stateType + ", state " + state);
					...
					final String errorInfo = String.format(
						...
						String.format(
						configed.getResourceValue("InstallationStateTableModel.contradictingProductRequirements2"),
						existingRequest
						);
		...
		//logging.info(this, "checkForContradictingAssignments ===  " + info + " client " + clientId + ", actualproduct " + actualProduct + ", product " + product + ", stateType " + stateType + ", state " + state);
	...
	private void setChangedState(String clientId,  String product,  String stateType, String state)
	{
		...
		//if (!product.equals( actualProduct)) setValueAt( state, productsV.indexOf ( product ), 3 );
	...
	private void tellAndClearMissingProducts( String productId )
	{
		if (missingProducts.size() > 0)
		{
			...
			final de.uib.configed.gui.FShowList fMissingProducts 
			= new de.uib.configed.gui.FShowList
				(Globals.mainFrame, 
				Globals.APPNAME + ": " + configed.getResourceValue("InstallationStateTableModel.missingProducts.title"),
	...
	public void finishCollectiveChange()
	{
		...
		if (missingImplementationForAR.size() > 0)
		{
			...
			javax.swing.JOptionPane.showMessageDialog(	de.uib.configed.Globals.mainFrame,
				...
				+ products,
				configed.getResourceValue("InstallationStateTableModel.missingImplementationForActionRequest.title"),
				javax.swing.JOptionPane.INFORMATION_MESSAGE
												 );
		}
	...
	{
		combinedVisualValues.get(ProductState.KEY_actionRequest).put( productId, ar.toString() );
		
		int row = productsV.indexOf( productId );
		int col = displayColumns.indexOf( ProductState.KEY_actionRequest);
		logging.info(this, "setVisualActionRequest " + ar + " row " + row + " col " + col);
	...
	*/
	...
	public void collectiveChangeActionRequest( String productId, ActionRequest ar )
	{
		...
		logging.debug(this, "collectiveChangeActionRequest for product, changed products " + product2setOfClientsWithNewAction.keySet());
		
		// show the new settings for all products after recursion
		
		for (String prod : product2setOfClientsWithNewAction.keySet())
		{
			logging.debug(this, "collectiveChangeActionRequest for product  " + prod + " changed product for client number : " 
			...
			//logging.info(this, "collectiveChangeActionRequest, set value valid if set for all clients " +  ar + " for " + prod + " instead of " +
			//		combinedVisualValues.get(ProductState.KEY_actionRequest).get(prod));
				
			
			//if (      ( product2setOfClientsWithNewAction.get(prod) ).size() <    selectedClients.length    )
			//-- not each client got a new action for this product
			{
				//String oldVal = combinedVisualValues.get(ProductState.KEY_actionRequest).get( prod ); 
			...
			else
			{
				...
				for (String clientId : selectedClients)
				{
					...
					{
						if (!mixInVal.equals(  getChangedState(clientId, prod, ProductState.KEY_actionRequest) ))
							mixInVal = de.uib.configed.Globals.CONFLICTSTATEstring;
					}
					
					logging.info(this, "collectiveChangeActionRequest, set " +  ar + " for " + prod + " instead of " +
						combinedVisualValues.get(ProductState.KEY_actionRequest).get(prod) + ", or perhaps " 
						+ getChangedState(clientId, prod, ProductState.KEY_actionRequest) + " giving mixin "
						+ mixInVal);
					
				}
				//combinedVisualValues.get(ProductState.KEY_actionRequest).put (prod, ar.toString() );
		...
		//fireTableRowsUpdated(0, productsV.size()-1); //does not  trigger a new ordering command
	...
	protected void changeActionRequest (String product, String theActionRequestString)
	{
		...
		for (String productId : product2setOfClientsWithNewAction.keySet())
		{
			...
			// not each client got a new action for this product
			{
				...
				String mixedValue = mixToVisualState(ProductState.KEY_actionRequest, combinedVisualValues.get(ProductState.KEY_actionRequest), productId, (product2AR.get(productId)).toString());
				if (mixedValue.equals("mixed"))
				{
					logging.info(this, "changeActionRequest result mixed for product " + productId );
				}
					
			}
			else
			{
				combinedVisualValues.get(ProductState.KEY_actionRequest).put (productId, (product2AR.get(productId)).toString() );
			}
		}	
		
		if (missingProducts.size() > 0)
		{
			logging.info(this, "required by product " +  product + " but missing " + missingProducts);
			...
			{	
				...
				if (productStates != null)
				{
					...
					if (stateAndAction != null)
					{
						...
						logging.debug(this,"---- stateAndAction until now: ActionRequest for requiredProduct " + actionRequestForRequiredProduct);
						
						String installationStatusOfRequiredProduct = stateAndAction.get(InstallationStatus.KEY);
						
						logging.debug(this,"---- stateAndAction until now: InstallationStatus for requiredProduct " + installationStatusOfRequiredProduct);
						 ...
						 */
						{
							...
							checkForContradictingAssignments( clientId, requiredProduct, ActionRequest.KEY, ActionRequest.getLabel( requiredAR ));
							...
							{
								logging.info(this, "followRequirements:   no change of action request necessary for " + requiredProduct);
							}
							
							
							else
							{
								String alreadyExistingNewActionRequest = getChangedState( clientId, requiredProduct, ActionRequest.KEY  ); 
								
								if ( alreadyExistingNewActionRequest != null ) 
								{
									logging.info(this, "required product: '" +requiredProduct+ "'  has already been treated" );
									logging.info(this, "new action was " + alreadyExistingNewActionRequest);
				
									
									//logging.error(this, "recursion stop,  setting  "  + requiredAction + " but already set for clientId, product " 
									//		+ clientId + "," + requiredProduct + " : "  + alreadyExistingNewActionRequest);
								}
								else
								{
				
									logging.info(this, "ar:   ===== recursion into " + requiredProduct);
									recursivelyChangeActionRequest(clientId, requiredProduct, new ActionRequest(requiredAR));
								}
		...
		*/
		...
		switch(col)
		{
			...
			case 8 : 
				...
				//	logging.info(this, "value for firefox is " + combinedVisualValues.get(ProductState.KEY_actionRequest).get(actualProduct));
			...
			case 12:   
				String  serverProductVersion = (String) getGlobalProductInfos().get(actualProduct).get(de.uib.opsidatamodel.productstate.ProductState.KEY_versionInfo);
	...
	*/
	...
	public void setValueAt(Object value, int row, RowCondition rowCondition, int col)
	{
	...
	*/
	//it would require the existence of a complete deep copy of the original loaded data
	/*
	public void reset()
	{
	...
	*/
	...
	public void setActionRequestWithCondition(ActionRequest ar,  IntPredicate rowCondition)
	{
		logging.info(this, "setActionRequestWithCondition " + ar + " for " + getRowCount());
