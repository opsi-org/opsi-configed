	...
	*/
	...
	private Integer getIndex(java.util.List<String> list, String element)
	{
		...
		*/
		...
		//if (j != null && result != -1 &&  result != j) logging.warning(this, "getIndex, for swIdent " + swIdent + " differs indexOf " + result + " from mapped value "  + j);
		
		if ( j == null) 
		{
			logging.info(this, "getIndex, probably because of an upper-lower case or a null issue, not found for  " + element);
		}
		else
			result = j;
		...
		{
			//logging.info(this, "current listsize " + list.size() + " indexOfIgnoreCase for " + swIdent ); 
			int i = 0;
			while (result == -1 && i < list.size())
			{
				if (list.get(i).equalsIgnoreCase( element ) )
