...
public class AuditSoftwareXLicencePool extends Relation {
	/*
	 * AUDIT_SOFTWARE_TO_LICENSE_POOL in database | Field | Type | Null | Key |
	 * licensePoolId | varchar(100) | NO | MUL | NULL | name | varchar(100) | NO |
	 * PRI | NULL | version | varchar(100) | NO | PRI | NULL | subVersion |
	 * varchar(100) | NO | PRI | NULL | language | varchar(10) | NO | PRI | NULL |
	...
	/*
	 * private String produceSWident(Map<String, String> m) { String result =
	...
	/*
	 * private Integer produceSWidRaw(Map<String, Object> m) { return
	 * produceSWid(new RelationElement(m)); } private Integer
	 * produceSWid(Map<String, String> m) { String swIdent = produceSWident(m); int
	 * result = -1; boolean newEntry = false; logging.info(this, "produceSWid for "
	 * + swIdent); int swId = registeredSoftware.indexOf(swIdent); if (swId > -1)
	 * result = swId; else { //logging.error("no software entry for " + swIdent);
	 * registeredSoftware.add(swIdent); result = registeredSoftware.size(); newEntry
	 * = true; } String info = ""; if (m.get(LicencepoolEntry.idSERVICEKEY) != null)
	 * info = " " + m.get(LicencepoolEntry.idSERVICEKEY) + " "; //logging.info(this,
	 * "registeredSoftware " + registeredSoftware); logging.info(this, "swident  " +
	 * swIdent + " for " + info + "  ==== id (new " + newEntry +") " + result);
	 * return result; }
	 */
