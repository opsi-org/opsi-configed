...
public class HostInfo 
{
	...
	*/
	...
	public void showAndSaveInternally(JTableSelectionPanel selectionPanel, MainFrame mainFrame, PersistenceController persist,
	                        String client, Map<String, String> sourceOfChanges)
	{
		...
		if (  (sourceOfChanges.get(clientIpAddressKEY) != null) && !(sourceOfChanges.get(clientIpAddressKEY).trim()).equals("") )
		{
			clientIpAddress = (sourceOfChanges.get(clientIpAddressKEY)).trim();
			
			int col = findCol(selectionPanel, configed.getResourceValue("ConfigedMain.pclistTableModel.clientIPAddress"));
			if (col > -1) selectionPanel.getTableModel().setValueAt( clientIpAddress, row, col);
