...
public class HostUpdate implements UpdateCommand {
	...
	public void doCall() {
		...
		// Before manipulating the data, we'll create a copy, because
		// we should not manipulate the original data
		Map<String, Object> settings = new HashMap<>(newdata);
		...
		settings.put("id", depot);

		// So the entry "maxBandwith" is actually an integer, but we can only edit as a string,
		// so we have to parse it here
		if (settings.containsKey("maxBandwidth")) {
			settings.put("maxBandwidth", Integer.parseInt(settings.get("maxBandwidth").toString().strip()));
		}

		// Without the type, the method will not work on the server
		settings.put("type", type);

		persistenceController.getDataServices().host.setHostValues(settings);
	}
}
