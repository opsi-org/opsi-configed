...
public class PanelSWInfo extends JPanel
{
	...
	public enum KindOfExport { PDF, CSV };
		...
		*/
		...
		layoutSubPanelTitle.setHorizontalGroup( layoutSubPanelTitle.createSequentialGroup()
		                                        ...
		                                        .addGroup(layoutSubPanelTitle.createParallelGroup()
		                                                  .addComponent(labelSuperTitle, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
	...
	protected void buildPanel()
	{
		...
		/*logging.info(this, "voidTableModel cols hopefully   " + SWAuditEntry.KEYS);//.getKeys());
		for (int i = 0; i<voidTableModel.getColumnCount(); i++)
		{
			logging.info(this, "voidTableModel col " + i + " " + voidTableModel.getColumnName(i));
		}
		System.exit(0);
		*/
		...
		layoutEmbed.setHorizontalGroup(
		    ...
		    .addGroup(layoutEmbed.createParallelGroup()
		              ...
		              .addGroup(layoutEmbed.createSequentialGroup()
		              	   .addGap(hGap, hGap, hGap)
		              	   .addComponent(jlabelSuperTitle,  javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
		              	   .addGap(hGap, hGap, hGap)
		              	)
		              */
		              //.addComponent(scrollPaneSWInfo, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
		...
		if (withPopup)
		{
			...
			popupTrait.addPopupListenersTo(new JComponent[]{this, panelTable.getTheTable(), panelTable.getTheScrollpane(), jTable, scrollPaneSWInfo, scrollPaneSWInfo.getViewport()});
	...
	public void sendToTerminal(  )
	{
		...
		de.uib.configed.gui.swinfopage.SWterminalExporter exporter = new de.uib.configed.gui.swinfopage.SWterminalExporter( PersistenceControllerFactory.getPersistenceController() );
	...
	public void sendToPDF(  )
	{
		...
		// DocumentToPdf tableToPDF = new DocumentToPdf(clientName, metaData); //  no filename, metadata
		//the real filename is summoned in the toPDF method call 
		DocumentToPdf tableToPDF = new DocumentToPdf ("report_swaudit_" + clientName, metaData); //  no filename, metadata
		...
		//tableToPDF.createContentElement("tablemodel",  sourceModel); //we use the model in order to avoid any graphical component
		
		
		modelSWInfo.setSorting(0, true);
		tableToPDF.createContentElement("tablemodel",  modelSWInfo); //we use the model in order to avoid any graphical component
		tableToPDF.setPageSizeA4_Landscape();  //
		if (askingForKindOfAction)
			tableToPDF.toPDF(null, pdfFilename);// create Pdf, ask if it shall be shown or saved
		else
			tableToPDF.toPDF(true, pdfFilename );// create Pdf, and save it,  if exportFilename null, ask for it
		**/
	}
	...
	public void updateModel()
	{
		...
		//logging.info(this, "update+++++ voidTableModel.getRowCount() " +voidTableModel.getRowCount() );
