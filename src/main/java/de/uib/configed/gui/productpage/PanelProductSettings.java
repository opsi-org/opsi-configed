...
implements RowSorterListener
{
	...
	protected void init()
	{
		tableProducts = new JTable(){
			public void setValueAt(Object value, int row, int column)
			{
				...
				//only in case of setting ActionRequest needed, since we there call fireTableDataChanged
		...
		tableProducts.getSelectionModel().addListSelectionListener(new ListSelectionListener() 
		{
			...
			{
				//logging.info(this, "-----------------tableProducts ListSelectionListener valueChanged");
					...
					int column)
				{
					...
					{	
						...
						if (val.equals(InstallationStateTableModel.CONFLICTstring)
							||
							val.equals(InstallationStateTableModel.unequalAddstring + InstallationStateTableModel.CONFLICTstring)
						)
						{
		...
		*/
		...
		logging.info(this, " created properties Panel, is  EditMapPanelX instance No. " + ((EditMapPanelX) propertiesPanel).objectCounter);
	...
	protected void producePopupMenu(final Map<String, Boolean> checkColumns)
	{
		...
		//itemSaveAndExecute.setAccelerator( KeyStroke.getKeyStroke( KeyEvent.VK_X, InputEvent.CTRL_DOWN_MASK) );
