...
public class PanelHostConfig extends JPanel
{
	...
	private void handleUserInPropertyClass( String superclass, String user )
	{
		...
		/*
		if (! PersistenceControllerFactory.getPersistenceController().PROPERTYCLASSES_SERVER.containsKey(newpropertyclass))
		{
			logging.debug(this, "putUsersToPropertyclassesTreeMap found another user named " + user + " [" + newpropertyclass +"]");
			PersistenceControllerFactory.getPersistenceController().PROPERTYCLASSES_SERVER.put(newpropertyclass, "");
		}
		*/
	...
	private void putUsersToPropertyclassesTreeMap()
	{
		Map<String, Object> configs = PersistenceControllerFactory.getPersistenceController()
							.getConfig(PersistenceControllerFactory.getPersistenceController().getHostInfoCollections().getConfigServer());
