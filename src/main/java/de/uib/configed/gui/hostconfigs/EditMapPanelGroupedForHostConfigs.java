...
// works on a map of pairs of type String - List
{
		...
		MouseListener popupListener0 = new utils.PopupMouseListener(popup0){
			...
			protected void maybeShowPopup(MouseEvent e) {
				if (e.isPopupTrigger()) {
					...
					//if(selRow % 2 == 0) //test
					if ( selPath != null && !isUserPath( selPath ) && !isRolePath( selPath, false) &&  !isRolePath( selPath, true) )
						...
						else 
						{
							...
							// we really are in a user branch
							{
								...
								//rolekey may be edited
								{
									...
									java.util.List<Object> values = de.uib.opsidatamodel.PersistenceControllerFactory.getPersistenceController().
									...
									if (obeyToRole)
									{
										result = true;
										JOptionPane.showMessageDialog(de.uib.configed.Globals.mainFrame, 
											configed.getResourceValue("EditMapPanelGroupedForHostConfigs.noManualEditingWhereRoleDefined")
	...
	protected void addUser(String rolename)
	{
		...
		de.uib.opsidatamodel.PersistenceController persist = de.uib.opsidatamodel.PersistenceControllerFactory.getPersistenceController();
	...
	private void buildUserConfig()
	{
		...
		de.uib.opsidatamodel.permission.UserConfigProducing up 
			= new de.uib.opsidatamodel.permission.UserConfigProducing(
