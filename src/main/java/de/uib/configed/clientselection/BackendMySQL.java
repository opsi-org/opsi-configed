...
public class BackendMySQL {
	...
	List hwConfig;
	// Für die Verbindung zum lokalen mysql
	//Connection conn;
	//Statement stat;
	...
	public BackendMySQL(PersistenceController controller) {
		...
		// Wird nur fürs lokale Testen benötigt
		/*try {
			conn = getConnection();
		...
		//for(int i=0; i<hwConfig.size(); i++) {
			...
			/*for(int j=0; j<((List)((HashMap)hwConfig.get(i)).get("Values")).size(); j++) {
				logging.info(this, ((HashMap)((List)((HashMap)hwConfig.get(i)).get("Values")).get(j)).get("Type"));
				logging.info(this, ((HashMap)((List)((HashMap)hwConfig.get(i)).get("Values")).get(j)).get("Opsi"));
				logging.info(this, ((HashMap)((List)((HashMap)hwConfig.get(i)).get("Values")).get(j)).get("WMI"));
				logging.info(this, ((HashMap)((List)((HashMap)hwConfig.get(i)).get("Values")).get(j)).get("Scope"));
				logging.info(this, ((HashMap)((List)((HashMap)hwConfig.get(i)).get("Values")).get(j)).get("Linux"));
				logging.info(this, ((HashMap)((List)((HashMap)hwConfig.get(i)).get("Values")).get(j)).get("OSX"));
				//
			}
			
			//
			//
			logging.info(this, ((HashMap)((HashMap)hwConfig.get(i)).get("Class")).get("Opsi").getClass());
			logging.info(this, ((HashMap)((HashMap)hwConfig.get(i)).get("Class")).get("WMI").getClass());
			logging.info(this, ((HashMap)((HashMap)hwConfig.get(i)).get("Class")).get("Linux").getClass());
			logging.info(this, ((HashMap)((HashMap)hwConfig.get(i)).get("Class")).get("UI").getClass());
			logging.info(this, ((HashMap)((HashMap)hwConfig.get(i)).get("Class")).get("OSX").getClass());
	...
	public List<String> getListFromSQL(String abfrage) {
		...
		java.util.List<java.util.List<java.lang.String>> clients
	...
	public List<String> getListFromJSONObject(JSONObject jsonObject) {
		...
		try {
			if(jsonObject.isNull("element")) {
				...
				switch(jsonObject.getString("operation")) {
					...
					case "SoftwareOperation": // PRODUCT
						...
						// Diese Abfrage liefert alle hostIds, die ein 'passendes' Produkt in PRODUCT_ON_CLIENT haben
						...
						// Diese Abfrage soll alle hostIds liefern, die ein 'passendes' Produkt haben, das nicht in PRODUCT_ON_CLIENT ist. Entsprechend werden die Standardwerte abgefragt.
						String abfrage2 = "SELECT DISTINCT HOST.hostId FROM HOST CROSS JOIN PRODUCT d WHERE ( " + where_clause2 + " ) AND NOT EXISTS (SELECT null FROM PRODUCT_ON_CLIENT WHERE d.productId LIKE PRODUCT_ON_CLIENT.productId AND hostId LIKE PRODUCT_ON_CLIENT.clientId);";
					...
					case "PropertiesOperation":
						...
						abfrage = "SELECT DISTINCT HOST.hostId FROM HOST INNER JOIN PRODUCT_PROPERTY_STATE h ON (h.objectId LIKE HOST.hostId) WHERE " + where_clause + ";";
						...
						abfrage2 = "SELECT DISTINCT HOST.hostId FROM HOST INNER JOIN PRODUCT_PROPERTY_VALUE v WHERE v.isDefault LIKE '1' AND (" + where_clause2 + ") AND NOT EXISTS (SELECT null FROM PRODUCT_PROPERTY_STATE WHERE v.productId LIKE productId AND HOST.hostId LIKE objectId AND v.propertyId LIKE propertyId);";
					...
					case "SoftwareWithPropertiesOperation":
						...
						String where_clause3 = getWhereClauseDefaultProductProperty(where_clause); // v an die Abfragen schreiben, da hier in
						String where_clause4 = getWhereClauseDefaultProductProperty(where_clause2); // PRODUCT_PROPERTY_STATE v abgefragt wird
						
						// Diese Abfrage soll alle hostIds liefern, die ein 'passendes' Produkt haben, das nicht in PRODUCT_ON_CLIENT ist, aber
						...
						abfrage2 = "SELECT DISTINCT HOST.hostId FROM HOST INNER JOIN PRODUCT_PROPERTY_STATE h ON (h.objectId LIKE HOST.hostId) INNER JOIN PRODUCT d ON (d.productId LIKE h.productId) WHERE (" + where_clause2 + ") AND NOT EXISTS (SELECT null FROM PRODUCT_ON_CLIENT WHERE h.productId LIKE PRODUCT_ON_CLIENT.productId AND d.productVersion LIKE PRODUCT_ON_CLIENT.productVersion AND d.packageVersion LIKE PRODUCT_ON_CLIENT.packageVersion AND hostId LIKE PRODUCT_ON_CLIENT.clientId);";
						
						
						
						// Diese Abfrage soll alle hostIds liefern, die eine 'passende' Product-Property haben, die nicht in PRODUCT_PROPERTY_STATE ist,
						...
						where_clause3 = where_clause3.replace("\"%", "");
						String abfrage3 = "SELECT DISTINCT HOST.hostId FROM HOST INNER JOIN PRODUCT_ON_CLIENT d ON (HOST.hostId=d.clientId) INNER JOIN PRODUCT_PROPERTY_VALUE v ON (v.productId LIKE d.productId) WHERE v.isDefault LIKE '1' AND (" + where_clause3 + ") AND NOT EXISTS (SELECT null FROM PRODUCT_PROPERTY_STATE WHERE d.productId LIKE productId AND HOST.hostId LIKE objectId AND v.propertyId LIKE propertyId);";
						
						
						// Diese Abfrage dient der Suche nach hostIds, die ein 'passendes' Produkt haben, das nicht in PRODUCT_ON_CLIENT ist, und eine 
						// 'passende' PRODUCT_PROPERTY, die nicht in PRODUCT_PROPERTY_STATE ist.
						where_clause4 = where_clause4.replace("%\"", "");
						where_clause4 = where_clause4.replace("\"%", "");
						
						String abfrage4 = "SELECT DISTINCT HOST.hostId FROM HOST CROSS JOIN PRODUCT_PROPERTY_VALUE v INNER JOIN PRODUCT d ON (d.productId LIKE v.productId AND d.productVersion LIKE v.productVersion AND d.packageVersion LIKE v.packageVersion) WHERE v.isDefault like '1' AND ( " + where_clause4 + " ) AND NOT EXISTS (SELECT null FROM PRODUCT_ON_CLIENT WHERE v.productId LIKE PRODUCT_ON_CLIENT.productId AND hostId LIKE PRODUCT_ON_CLIENT.clientId) AND NOT EXISTS (SELECT null FROM PRODUCT_PROPERTY_STATE WHERE v.productId LIKE productId AND HOST.hostId LIKE objectId AND v.propertyId LIKE propertyId);";
					...
					case "HardwareOperation":
						...
						abfrage = "SELECT DISTINCT HOST.hostId FROM HOST " + inner_joins + " WHERE i.state LIKE 1 AND (" + abfrage + ")";
			...
			} else if(jsonObject.getJSONArray("elementPath").getString(0).equals("GroupWithSubgroups")) { // Group with subgroups
	...
	private List<String> getGroupWithSubgroup(String groupname) {
		List<String> subgroups = getListFromSQL("SELECT groupId FROM `GROUP` WHERE parentGroupId LIKE '" + groupname + "'");
		
		List<String> clients = getListFromSQL("SELECT hostId FROM HOST INNER JOIN OBJECT_TO_GROUP a ON (HOST.hostId=a.objectId AND a.groupType LIKE 'HostGroup' ) WHERE a.groupId LIKE '" + groupname + "'");
	...
	// Es müssen entsprechend noch die Standardwerte anstelle der Platzhalter  ergänzt werden.
	public String getWhereClauseDefaultProduct(String abfrage) {
		...
		abfrage = abfrage.replace("DATE(d.modificationTime)", "'IRGENDWAS MIT DATUM'"); // Was nie modifiziert wurde, soll hier auch nie kommen
	...
	public String getWhereClauseDefaultProductProperty(String abfrage) {
		...
		// Weil in PRODUCT_PROPERTY_VALUE 'false' als 0 und 'true' als 1 abgespeichert wird
	...
	/* Der Teil hier wird eigentlich nur für das Testen mit der lokalen mySQL-Datenbank benötigt
	...
	public static String getAbfrage(int lineNumber) {
		try {
			String line;
			BufferedReader bufferedReader = new BufferedReader(new FileReader("gespeicherte_abfragen.txt"));
	...
	*/
	...
	public List<String> getClientListFromJSONString(String abfrage) {
		...
		} catch(JSONException e) {logging.warning(this, "" + e);}
	...
	private String doJSONObject(JSONObject jsonObject, boolean first, MySQL.Type type) {
		...
		if(jsonObject.isNull("element")) {
			...
			} catch(Exception e) {logging.warning(this,"" + e);}
