...
public class BackendMySQL {
	...
	public List<String> getListFromJSONObject(JSONObject jsonObject) {
		...
		MySQL mySQL_rekursion = new MySQL(hwConfig);
		...
		try {
			if (jsonObject.isNull("element")) {
				...
				case "SoftwareOperation": // PRODUCT
					...
					String where_clause = doJSONObject(mySQL_rekursion, jsonObject, true, MySQL.Type.NEW);
				...
				case "PropertiesOperation":
					...
					where_clause = doJSONObject(mySQL_rekursion, jsonObject, true, MySQL.Type.NEW);
				...
				case "HardwareOperation":
					abfrage = doJSONObject(mySQL_rekursion, jsonObject, true, MySQL.Type.NEW);
	...
	private String doJSONObject(MySQL mySQL_rekursion, JSONObject jsonObject, boolean first, MySQL.Type type) {

		if (jsonObject.isNull("element")) {
			MySQL.Type newType = MySQL.getType(jsonObject);
			try {
				return doJSONArray(mySQL_rekursion, (JSONArray) jsonObject.getJSONArray("children"), newType);
	...
	private String doJSONArray(MySQL mySQL_rekursion, JSONArray jsonArray, MySQL.Type type) {
		...
		for (int i = 0; i < length; i++) {
			try {
				...
				mysql += doJSONObject(mySQL_rekursion, jsonObject, i == 0, MySQL.Type.NEW);
			} catch (Exception e) {
				logging.warning(this, "" + e);
			}
		}
