    ...
    public void scan() throws IOException
    {
        ...
        while ((currentLine = reader.readLine()) != null)
        {
            ...
            for (int i = 0; i < cReader.size(); i++)
            {
                ...
                {
                    if ((cReader.peek() == format.getFieldSeparator() || i == currentLine.length() - 1) && fieldBeginsWithEmbeddedQuote)
                    {
                        fieldBeginsWithEmbeddedQuote = false;
                        CSVToken emptyFieldToken = new CSVToken(CSVToken.EMPTY_FIELD, String.format("%c%c", format.getStringSeparator(), format.getStringSeparator()));
                        lineOfTokens.add(emptyFieldToken);
                        continue;
                    }

                    if ((cReader.lastRead() == format.getStringSeparator() && i == 0 || cReader.lastRead() == format.getFieldSeparator()) && cReader.peek() == format.getStringSeparator())
                    {
                    ...
                    if (cReader.peek() == format.getStringSeparator() && i != currentLine.length() - 1)
                    {
                        ...
                        {
                            CSVToken stringSeparatorToken = new CSVToken(CSVToken.STRING_SEPARATOR, String.valueOf(format.getStringSeparator()));
                            lineOfTokens.add(stringSeparatorToken);
                            CSVToken embeddedQuoteToken = new CSVToken(CSVToken.EMBEDDED_QUOTE, String.valueOf(format.getStringSeparator()));
                ...
                {
                    CSVToken fieldSeparatorToken = new CSVToken(CSVToken.FIELD_SEPARATOR, String.valueOf(format.getFieldSeparator()));
