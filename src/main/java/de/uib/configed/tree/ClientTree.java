		...
		
{
	...
	{
		super();
		setToggleClickCount(0); //do not expand tree nodes when clicking the node name, default is 2, meaning double click expands
	...
	*/
	...
	public void keyPressed( KeyEvent e)
	{
		...
		//logging.info(this, "key pressed on row " + " selected " + Arrays.toString(selection) + "\n" + e);
	...
	public void mousePressed(final MouseEvent e) 
	{
		...
		if (!mouse_ready)
		{
			...
			//logging.debug(this, "mousePressed, but mouse not ready setSelectedClientsArray");
			return;
		}
		
		//logging.debug(this, "mousePressed, mouse ready "); // setSelectedClientsArray");
		...
		int selRow = getRowForLocation(e.getX(), e.getY());
		//logging.debug(this," mouse pressed,  (x,y) " + e.getX() + ", " + e.getY()  + "  row " + selRow);
	...
	*/
	...
	public Set<String> associateClientsToGroups(String[] x, Map<String, Set<String>> fObject2Groups, Set<String> permittedHostGroups)
	{
		...
		for ( String clientId : x )
		{
			if (fObject2Groups.get(clientId) != null)
			{
				...
				//logging.info(this, "--------------- client " + clientId + " belongs to groups " + belongingTo);
			...
			for (String clientId : leafname2AllItsPaths.keySet() )
			{
				for (SimpleTreePath path : leafname2AllItsPaths.get( clientId ) )
				{
					...
					if (notPermittedNumber<  allElementsNumber )
					{
						...
						//+ clientId + " not " + notPermittedNumber + " all " + allElementsNumber +  " : " + (notPermittedNumber<  allElementsNumber ) );
		...
		/*
		logging.info(this, "associateClientsToGroups allowed clients " + allowedClients);
	...
	*/
	...
	public HashSet<String> collectParentIDs(String nodeID)
	{
		...
		//logging.info(this,  "collectParentIDs for node " + nodeID  + " treePaths " + treePaths);
		
		if (treePaths == null)
		{
			//logging.debug(this, "collectParentIDs nodeID, treePaths " + nodeID + ", " + treePaths);
		}
		else
		{
		...
		//logging.info(this,  "collectParentIDs for node " + nodeID  + " allParents " + allParents);
