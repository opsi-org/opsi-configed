			...
			, ItemListener 				// for combos
{
	...
	//protected de.uib.configed.gui.IconButton buttonFilter; //instead of it, we use the filter icon of the tablesearchpane
	...
	protected void initComponents()
	{
		...
		buttonCancelStateEditing = new de.uib.configed.gui.IconButton( 
			de.uib.configed.configed.getResourceValue("GroupPanel.CancelStateEditingTooltip") , 
			"images/cancel_light.png", 
			"images/cancel_light.png", 
			"images/cancel_light.png");

		buttonCancelStateEditing.addActionListener(this);
		buttonCancelStateEditing.setPreferredSize(de.uib.utilities.Globals.newSmallButton);
		...
		*/
		...
		/*comboAggregatedEditing.addActionListener( new ActionListener(){
				...
				{
					if (tableProducts != null &&  (tableProducts.getModel() instanceof IFInstallationStateTableModel))
					{
						logging.info(this, " action on comboAggregatedEditingresulting in  " + comboAggregatedEditing.getSelectedItem() );  
						handleCollectiveAction(  (IFInstallationStateTableModel) tableProducts.getModel() );
					}
				}
			}
		);*/
		...
		values.put(
			de.uib.configed.configed.getResourceValue("GroupPanel.comboAggregateProducts.defaultOption"), 
			de.uib.configed.configed.getResourceValue("GroupPanel.comboAggregateProducts.defaultOption.tooltip"));
			*/
		values.put(
		...
		//"setup (marked)", "set 'setup' for all marked products (if setup action exists)");

		/*
		values.put(
			de.uib.configed.configed.getResourceValue("GroupPanel.comboAggregateProducts.setupDiffering"),
			de.uib.configed.configed.getResourceValue("GroupPanel.comboAggregateProducts.setupDiffering.tooltip"));
			
			//"setup (differing)", "set 'setup' for all products with other than current product version (if setup action exists)");
		values.put(
			de.uib.configed.configed.getResourceValue("GroupPanel.comboAggregateProducts.setupFailed"),
			de.uib.configed.configed.getResourceValue("GroupPanel.comboAggregateProducts.setupFailed.tooltip"));
		...
		*/
		...
		//"uninstall (marked)", "set 'uninstall' for all marked products (if uninstall action exists)");


		DefaultListModel<String> modelChooseAction = new DefaultListModel<String>(); //put values from hashmap into list
		...
		JScrollPane scrollChooseAction = new JScrollPane(listChooseAction, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		surroundActionLayout.setVerticalGroup(
		    ...
		    .addGap( 30 ) // (int) (1.5 * Globals.lineHeight) ) for levelling the list when centering the components
		    .addComponent( labelStrip, 15, 15, 15 )
		    .addComponent(scrollChooseAction, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, 3*Globals.lineHeight)
		)
		;
		surroundActionLayout.setHorizontalGroup(surroundActionLayout.createParallelGroup()
			...
			.addGroup(
				surroundActionLayout.createSequentialGroup()
				.addGap( 2*Globals.gapSize )
				.addComponent(scrollChooseAction, 2*Globals.buttonWidth, 2*Globals.buttonWidth, Short.MAX_VALUE)
			)
		   )
		...
		buttonFilter.setVisible(false); //we use the filtericon of the TableSearchPane
		*/
		...
		*/
		...
		//request setup for all marked products and all selected clients if installed and outdated
		//change installed status to not_installed
		//remove the complete status information


		JLabel labelDeclareCombinedSetting = new JLabel("set for all marked products");
		...
		GroupLayout layoutPanelCombinedSettings =  new GroupLayout(panelCombinedSettings);
		...
		layoutPanelCombinedSettings.setVerticalGroup(
			layoutPanelCombinedSettings.createSequentialGroup()
				.addGap(Globals.vGapSize/2, Globals.vGapSize/2, Globals.vGapSize/2)
				.addGroup(layoutPanelCombinedSettings.createParallelGroup(GroupLayout.Alignment.BASELINE)
					.addComponent(labelSave, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
				)
		*/
		...
		JPanel separatingPlace = new JPanel(); separatingPlace.setForeground( Globals.backLightYellow ); separatingPlace.setBackground(Color.RED);
		...
		layoutMain.setVerticalGroup(
		    ...
		    .addGroup(layoutMain.createParallelGroup(GroupLayout.Alignment.CENTER)
		              ...
		              .addComponent(buttonReloadProductStates, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		              .addComponent(buttonSaveAndExecute, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		              
		              //.addComponent(buttonCancelStateEditing, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		              //.addComponent(buttonCollectiveAction, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		              //.addGroup( layoutMain.createSequentialGroup()
		              //.addComponent(labelCollectiveAction, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)

		              //.addGroup( layoutMain.createSequentialGroup()
		              //.addGap(Globals.vGapSize, Globals.vGapSize, Globals.vGapSize)
		              .addComponent(surroundScrollChooseAction, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, 3*Globals.lineHeight)
		              .addComponent(buttonEditDialog, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		              //.addComponent(menuBarAggregateActions, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		             )
		    .addGap(Globals.vGapSize/2, Globals.vGapSize/2, Globals.vGapSize/2)
		...
		layoutMain.setHorizontalGroup(
		    ...
		    .addGroup(layoutMain.createSequentialGroup()
		              ...
		              //.addComponent(labelCollectiveAction, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
		              //.addGap(0, Globals.gapSize/2, Globals.gapSize/2)
		              .addComponent(surroundScrollChooseAction, minComboWidth, GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
		              //.addGap(0, Globals.gapSize/2, Globals.gapSize/2)
		              //.addComponent(buttonCollectiveAction, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)
	...
	protected boolean save()
	{
		...
		else
		{
			...
			Set<String> extendedSelection =  mainController.getPersistenceController().extendToDependentProducts(  associate.getSelectedIDs(), "bonifax.uib.local" );
	...
	public void actionPerformed(java.awt.event.ActionEvent e)
	{
		...
		else if (e.getSource() == buttonCancelStateEditing)
		{
			...
			//insTableModel.setActionIf( ActionRequest.produceFromLabel("setup"),  i -> i > 0);
			insTableModel.reset();
	...
	protected void handleCollectiveAction(String selected, IFInstallationStateTableModel insTableModel )
	{
		...
		//insTableModel.setActionRequestWithCondition( new ActionRequest( ActionRequest.SETUP ),
		//	(java.util.function.IntPredicate) ( rowId -> associate.getSelectedRowsInModelTerms().indexOf( rowId  ) >= 0 )
		//	);
