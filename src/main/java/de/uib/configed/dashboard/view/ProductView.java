...
{
	@FXML private Label productsNumberLabel;
	@FXML private Label localbootProductsNumberLabel;
	@FXML private Label netbootProductsNumberLabel;
	@FXML private Label installedProductsNumberLabel;
	@FXML private Label failedProductsNumberLabel;
	@FXML private Label unusedProductsNumberLabel;
	@FXML private TextField clientSearchbarTextField;
	@FXML private TextField productSearchbarTextField;
	@FXML private CheckBox productStatusCheckBox;
	@FXML private ChoiceBox<String> productStatusChoiceBox;
	@FXML private ListView<String> clientListView;
	@FXML private TableView<Product> productTableView;
	@FXML private TableColumn<Product, String> productIdTableColumn;
	@FXML private TableColumn<Product, String> productStatusTableColumn;
	@FXML private TableColumn<Product, String> productInDepotTableColumn;
	@FXML private Button backButton;
	@FXML private ProductComparison productComparison;
	@FXML private ProductStatusComparison productStatusComparison;
	...
	{
		FXMLLoader fxmlLoader = new FXMLLoader(ProductView.class.getResource("/resources/fxml/views/product_view.fxml"), Messages.getResource());
	...
	private void loadData()
	{
		...
		productIdFilter.bind(Bindings.createObjectBinding(() ->
			product ->
			{
				if (productSearchbarTextField.getText() == null) return true;
				return product.getId().contains(productSearchbarTextField.getText());
			},
			productSearchbarTextField.textProperty()));
		productStatusFilter.bind(Bindings.createObjectBinding(() ->
			product ->
			{
				if (productStatusChoiceBox.getValue() == null) return true;
				return productStatusChoiceBox.getValue().equals(configed.getResourceValue("Dashboard.products.installed")) && product.getStatus().equals(configed.getResourceValue("Dashboard.products.installed")) ||
					   productStatusChoiceBox.getValue().equals(configed.getResourceValue("Dashboard.products.failed")) && product.getStatus().equals(configed.getResourceValue("Dashboard.products.failed")) ||
					   productStatusChoiceBox.getValue().equals(configed.getResourceValue("Dashboard.products.unused")) && product.getStatus().equals(configed.getResourceValue("Dashboard.products.unused"));
			},
			productStatusChoiceBox.valueProperty()));

		filteredData.predicateProperty().bind(Bindings.createObjectBinding(() -> productIdFilter.get().and(productStatusFilter.get()), productIdFilter, productStatusFilter));
	...
	private void bindDataToListView(List<String> data, ListView<String> view, TextField searchbar)
	{
		...
		productDataFilter.bind(Bindings.createObjectBinding(() ->
			product -> product.toLowerCase(Locale.ROOT).contains(searchbar.getText().toLowerCase(Locale.ROOT)),
			searchbar.textProperty()));

		view.setItems(filteredData);

		filteredData.predicateProperty().bind(Bindings.createObjectBinding(() ->
			productDataFilter.get(), productDataFilter));
	}
